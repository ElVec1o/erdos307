...
**Version 1.58.0 · 2 September 2026** · Erdős #307 is **open**; this repository holds the paper, its Lean formalization, and every program behind its numbers.
...
paper/   erdos307-core.pdf       — the paper (32 pp; the results)
         erdos307-companion.pdf  — the companion (104 pp; explorations, closures, the square sieve and the density theorem)
...
- **Lean:** `cd lean && lake exe cache get && lake build` (Lean / mathlib `v4.30.0`). Then
  `lake env lean Check.lean` prints the axiom dependencies of 432 declarations across all 63 modules
...
`lake env lean Check.lean` probes 432 declarations across 60 modules; all depend only on
