// sector_floorlist.rs -- emits sectors whose mass floor is a prescribed |P u Q|, with run parameters.
// Regression: target 60 at primes <= 47, omega <= 8 gives 250, matching sector_at60_list.rs.
...
struct Ctx { pool: Vec<u64>, all: Vec<u64>, wmax: usize, target: usize, hits: std::cell::RefCell<Vec<Vec<u64>>>, nodes: std::cell::Cell<u64> }
...
fn dfs(c: &Ctx, i: usize, cur: &mut Vec<u64>) {
    ...
    if !cur.is_empty() {
        ...
        if sg > 0.0 {
            ...
            if k != usize::MAX && cur.len() + k == c.target - 1 {
    ...
    for t in 0..=(c.wmax - cur.len()).min(c.pool.len() - i) {
        ...
        if kmin != usize::MAX && cur.len() + t + kmin <= c.target - 1 { feasible = true; break; }
    }
    if !feasible { return; }
...
fn main() {
    ...
    let target: usize = a[3].parse().unwrap(); // 1 + omega(d) + K(d) = target
    let pool: Vec<u64> = (2..=pcap).filter(|&p| is_prime(p)).collect();
    let all: Vec<u64> = (2..20000u64).filter(|&p| is_prime(p)).collect();
    let c = Ctx { pool: pool.clone(), all, wmax, target, hits: std::cell::RefCell::new(vec![]), nodes: std::cell::Cell::new(0) };
    let mut cur = vec![];
    dfs(&c, 0, &mut cur);
    let h = c.hits.borrow();
