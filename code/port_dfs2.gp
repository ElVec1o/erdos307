...
\\ Output: one line per PPN found, appended to runs/port_dfs2/hits.txt (resumable by rerun).
C = 20; LIM = 40; DEPTH = 12;
seen = Map();
rec(n) = if(!mapisdefined(seen,n), mapput(seen,n,1); write("runs/port_dfs2/hits.txt", [omega(n), n]); print("PPN ", omega(n), " ", n));
closure(A,a,pmax) = {
  my(N = A^2 + a, D);
  if(A > 10^LIM, return);
  D = divisors(N);
