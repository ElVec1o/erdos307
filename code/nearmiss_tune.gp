/* nearmiss_tune.gp -- push the near-miss frontier r(a) = sigma(a)sigma(a') towards 1 by
 * construction. prop:nearmiss reports max r = 0.5535 and smallest relative defect |r-1| = 0.4465,
 * both from sweeping a <= 1e7. The region near r = 1 lies above the barrier and cannot be swept,
 * but it can be entered by construction.
 *
 * Method. Force W = {2,3,5} into a' by
 *      w | a'  <==>  sum_{p|a} p^{-1} = 0 (mod w),                                    (F)
 * so sigma(a') = 31/30 + eps, where eps is the mass of the unforced primes of a'. Build a from
 * primes >= 7 and tune sigma(a) just below 30/31 so that sigma(a)(31/30 + eps) lands near 1.
 * a' must factor completely for r to be EXACT, so only candidates whose cofactor after trial
 * division is prime are accepted; otherwise r is only bounded below and |r-1| is not decided.
 *
 * Usage: gp -q -f nearmiss_tune.gp
 */
...

{
  ...
  print("tuning sigma(a) just below 30/31 = ", 1.0*TARGET, " against the forced sigma(a') >= 31/30");
