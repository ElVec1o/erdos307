...
This table documents the scripts the paper cites by name. `code/` holds 164 tracked files in total;
...
| `a9rate_consequences.sh` | Every place in the paper, README and this file that would need revisiting if `cor:a9rate` stopped being conjectural. Paragraph-oriented, so a phrase split across a line break cannot hide. | 20 paragraphs flagged, each tagged with which pattern fired. | ~1 s |
