...
| `splitmitm/split_mitm.rs` | Stage A of the partial-`T` prune: from `beta' = q alpha` and `beta - alpha = q alpha'`, `1 - alpha^2/D = sigma(T) sigma(S \ T)`, so `alpha^2 <= tau D` pins `sigma(T)` to a window of width about `tau / sqrt(Sigma^2 - 4)`. Schroeppel-Shamir enumerates the window at every `|T|` at once; each candidate and its complement is decided in exact 512-bit arithmetic. `--selftest` checks the enumerator against brute force, the window against a 24-prime toy base, and the exact test on a positive control; `--audit` plants random splits on real open bases and requires the production enumerator to return them. About 83 s per base at `tau = 2^-40`. Checkpointed and resumable. |
| `splitmitm/stageA_exact.gp` | Exact audit of Stage A output: every reported split re-decided by the single equation `alpha^2 + alpha' beta' = D` with `q = beta'/alpha` a prime above `max S`; every base checked for exactly one `DONE` line; candidates and hits summed. Controls: `{2,3,5}`/`T={5}` recognised as satisfying the equation with the non-prime tail `q = 1`; a planted bogus report flagged; random splits of a level-60 base fail. |
| `qsieve/q_sieve.rs` | The balanced regime `alpha^2 > 2^-40 D` of the split sieve, decided over the tail prime: `q <= Sigma 2^40` and `(q|p) = ((D/p)|p)` at every odd `p` in the base, from `Aq + D = (q alpha + beta)^2`, `Bq + D = (q alpha - beta)^2` (`prop:tailbound`). One-variable wheel sieve, about 19 s per base; `--control` plants a `q0` and requires it back. Survivors are candidates for the exact square test. Checkpointed and resumable. |
| `qsieve/q_sieve2.rs` | Independent second implementation of `q_sieve.rs`: targets as products of Jacobi symbols by reciprocity, wheel `{2,3,5,11,13,17,19,23}`, bitset tables tested largest prime first, `qmax` in `u128` fixed point. On base 3 with planted `q0 = 1000003` it returns the same 125 survivors as `q_sieve.rs`, and the same empty lists on bases 3, 6, 14; candidate counts differ by the predicted wheel factor 1.116. Checkpointed. |
| `qsieve/q_exact.gp` | Exact decision of q-sieve survivors: `Aq + D` and `Bq + D` both squares, then the split recovered from their roots and checked against both cycle equations. Controls: accepts `{2,3,5}` and `{2,3,7,41}` at `q = 1` (rejecting the spurious `alpha = 6`), no double squares for `q` in `[2, 2000]` on `{2,3,5}`. |
