// sector42_k64.rs -- decide whether omega(e) = 64 is possible in the d = 42 sector of the two-cycle.
// Build: rustc -O -o sector42_k64 sector42_k64.rs      Run: ./sector42_k64 <phase 1|2> [threads]
...
struct Ctx { allowed: Vec<u64>, inv: Vec<DD>, invf: Vec<f64>, pre_max: Vec<Vec<f64>>, suf_min: Vec<Vec<f64>>, t: DD,
...
fn phase2_leaf(ctx: &Ctx, s: &mut Vec<u64>, sig: DD) {
    ...
    if delta.hi <= 1e-11 {
        ...
        let pos = r.mul_small(42).cmp(&rp.mul_small(41)) == std::cmp::Ordering::Greater;
    ...
    let lo2 = ((1.0 / delta.hi).floor() as u64).max(1230); let hi2 = (2.0 / delta.hi).floor() as u64 + 2;
...
fn main() {
    let args: Vec<String> = std::env::args().collect(); let phase: u32 = args[1].parse().unwrap_or(0);
    if args[1] == "selftest" { let mut al = vec![]; let mut p = 5u64; while al.len() < 63 { if is_prime_small(p) && p != 7 && p != 41 { al.push(p); } p += 1; }
        let mut r = Big::from_u64(1); let mut rp = Big::from_u64(0); for &p in &al { rp = rp.mul_small(p).add(&r); r = r.mul_small(p); }
        let a = r.mul_small(42); let b = rp.mul_small(41); let num = r.mul_small(41).add(&Big::from_u64(1764));
        let (q, rem) = num.divrem(&b); println!("R={}\nRp={}\n42R={}\nnum={}\nnum div 41Rp: q={} rem={}", r.to_string(), rp.to_string(), a.to_string(), num.to_string(), q.to_string(), rem.to_string()); return; }
 let nthreads: usize = args.get(2).map(|s| s.parse().unwrap()).unwrap_or(8);
    let mut allowed = vec![]; let mut p = 5u64; while allowed.len() < 400 { if is_prime_small(p) && p != 7 && p != 41 { allowed.push(p); } p += 1; }
    let n = allowed.iter().position(|&q| q >= 1231).unwrap(); allowed.truncate(n);
    ...
    const SPLIT0: usize = 44; const D: usize = 16;
    ...
    let mut pre_max = vec![vec![0.0; 65]; n + 1]; let mut suf_min = vec![vec![0.0; 65]; n + 1];
    for i in 0..n { let mut rem: Vec<f64> = invf[i..].to_vec(); rem.sort_by(|a, b| b.partial_cmp(a).unwrap());
        for c in 1..=64 { if c <= rem.len() { pre_max[i][c] = rem[..c].iter().sum(); suf_min[i][c] = rem[rem.len() - c..].iter().sum(); } else { pre_max[i][c] = f64::INFINITY; suf_min[i][c] = f64::INFINITY; } } }
    let t = dd_ratio(42.0, 41.0);
    let (k, lo, hi, phase2) = if phase == 1 { (63usize, t.hi - 1.0 / allowed[63] as f64, t.hi, false) } else { (62usize, t.hi - 2.0 / 1231.0, t.hi, true) };
    ...
    let ctx = Ctx { allowed, inv, invf, pre_max, suf_min, t, leaves: AtomicU64::new(0), in_window: AtomicU64::new(0), exact: AtomicU64::new(0), found: AtomicU64::new(0), min_delta: AtomicU64::new(f64::INFINITY.to_bits()), max_hi2: AtomicU64::new(0),
