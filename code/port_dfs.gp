\\ Primitive port fillings for Erdos #313 (primary pseudoperfect numbers).
\\ Node = squarefree A with defect a = A - A' > 0 (sigma(A) < 1). Appending a prime p > A/a
\\ gives defect a*p - A. Defect 1 <=> PPN. DFS keeps defects <= B; at every node with
\\ A < 10^LIM the two-prime closure (a q1 - A)(a q2 - A) = A^2 + a is solved by factoring.
\\ Output: one line per PPN found, appended to runs/port_dfs/hits.txt (resumable by rerun).
B = 2000; LIM = 36; DEPTH = 12;
seen = Map();
rec(n) = if(!mapisdefined(seen,n), mapput(seen,n,1); write("runs/port_dfs/hits.txt", [omega(n), n]); print("PPN ", omega(n), " ", n));
closure(A,a,pmax) = {
  my(N = A^2 + a, D);
  if(A > 10^LIM, return);
  D = divisors(N);
  for(i=1, #D, my(d=D[i], e=N/d, q1, q2);
    if(d >= e, break);
    if((d + A) % a || (e + A) % a, next);
    q1 = (d + A)/a; q2 = (e + A)/a;
    if(q1 > pmax && isprime(q1) && isprime(q2), rec(A*q1*q2)));
}
dfs(A, a, pmax, k) = {
  if(a == 1, rec(A));
  if(k >= DEPTH, return);
  closure(A, a, pmax);
  my(lo = max(pmax+1, A\a + 1), hi = (A + B)\a);
  forprime(p = lo, hi, dfs(A*p, a*p - A, p, k+1));
}
forprime(p=2, B+1, dfs(p, p-1, p, 1));
print("DONE nodes-limited run B=",B," LIM=",LIM);
