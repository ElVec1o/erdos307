...
\date{Version 1.66.0\quad 2 September 2026}
...
to $2$, so $(\sigma+2)t+1<5.07$ and $x<2.26\,D_S<(\sigma+2)D_S=A_S$. Hence $x$ lies in $[0,A_S)$ and,
$A_S$ being prime, is one of the two roots of $X^2\equiv D_S$. Each root gives one candidate
$q=(x^2-D_S)/A_S$, to be tested for integrality, primality and the companion condition that
$B_Sq+D_S$ be a square. Carrying this out on the $34$ families with $A_S,B_S$ both Baillie--PSW
probable prime, every candidate fails \textup{(\texttt{code/immune\_decide.gp})}: at
\texttt{base\#64}, for instance, $\sigma=2.000253$, $A_S/D_S=4.000253$ and $x\le2.2504\,D_S$, and both
roots give a composite $q$. The conclusion is conditional on the primality of $A_S$ exactly as the
immunity itself was.
\end{proof}

What remains at level $60$ is therefore the $3{,}444$ families whose $A_S$ is composite and not split,
where the argument above needs the factorisation it does not have. Factoring the balanced semiprime
costs on the order of twenty core--years for all of them and decides at best half, since for $A=pq$
with $(D_S\mid A)=+1$ either both Legendre symbols are $+1$, when no kill exists, or both are $-1$,
when one does.
