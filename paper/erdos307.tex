...
$10^9$ (already found by $\rho$, so \emph{no} free kills), and Baillie--PSW primality leaves exactly
$34$ families with both $A_S,B_S$ prime --- these are \emph{provably immune} (a prime $A_S$ with
$(D_S\mid A_S)=+1$ has no inert factor, so no congruence obstruction for any $q$). A complete
kill/immune split of the residual is therefore beyond feasible computation; the honest state of the
single--tail sector is \emph{$46{,}483$ empty, $34$ provably immune, $3{,}478$ factoring--hard}.
\end{proof}
...
provably immune families together with the pair sector. This does not touch the reliability of the
