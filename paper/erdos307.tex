...
entry region has now been swept: over the $53{,}898$ squarefree $N_0\le10^{14}$ with
$\sigma(N_0)\ge\tfrac32$, the form $Q_{N_0}$ represents $4N_0^2$ not once, so there is no live slot
and no ghost either \textup{(\texttt{code/live\_slot\_census.rs})}. The primorial bases were
re-checked independently: $\Pi_{10},\dots,\Pi_{13}$ each admit no representation. The same program run without the
