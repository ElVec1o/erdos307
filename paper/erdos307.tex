...
the first valid unconditional bound of its kind, and establishing that required auditing what was
already there. Kovi\v{c} \cite{Kovic2012} states three bounds on an $n''=n$ solution. A computer
...
tail prime is bounded by $4N$. What follows is the orbit, the three gates, and the structural points
that survive.
...
supplies no prime term past $q\le4N$.
...
After this note was drafted, three items came to light; having now examined each directly, we record
...
A third item, older than both and not examined at drafting. We had cited Proposition~17 as the
comparison point for Theorem~\ref{thm:barrier}. Reading the paper directly turned up Proposition~18,
which claims strictly more --- the unconditional $r+s\ge34$, with $r+s\ge57$ if $\min\{p_1,q_1\}\ge3$
and $r+s\ge110$ if $\min\{p_1,q_1\}\ge5$, together with $p_1<r$, $q_1<s$ and
$\max\{m,n\}\ge\prod_{i=1}^{17}p_i\approx1.92\times10^{21}$. We record why we do not treat it as
prior art for the cardinality bound.

The proof opens with two correct pairs of bounds, which after substituting $n'=m$, $m'=n$ read
\[ \text{(A)}\quad \frac{rn}{p_r}<m<\frac{rn}{p_1}, \qquad\qquad
   \text{(B)}\quad \frac{sm}{q_s}<n<\frac{sm}{q_1}, \]
and continues ``Hence: $p_1q_s<rs$ and $q_1p_r<rs$.'' Those two carry everything downstream, via
$2\max\{p_r,q_s\}<N^2/4$ against $\max\{p_r,q_s\}\ge P_N$. But multiplying inequalities is sound only
when the directions agree, and from (A) and (B) exactly two products are available:
\[ \text{upper}\times\text{upper}:\ p_1q_1<rs, \qquad
   \text{lower}\times\text{lower}:\ rs<p_rq_s. \]
The asserted $p_1q_s<rs$ pairs an upper bound on $m$, which carries $p_1$, with a lower bound on $n$,
which carries $q_s$; the directions do not compose. Nor is the step recoverable. Writing
$\sigma(n)=n'/n$, the cycle says precisely that $\sigma(n)\in(r/p_r,\,r/p_1)$ and that
$\sigma(m)=1/\sigma(n)\in(s/q_s,\,s/q_1)$, i.e.\ that $\sigma(n)$ lies in
$(r/p_r,\,r/p_1)\cap(q_1/s,\,q_s/s)$. Nonemptiness of that intersection is equivalent to the
conjunction of the two valid products above and to nothing further, whereas $p_1q_s<rs$ is
$q_s/s<r/p_1$, a comparison of the two \emph{upper} endpoints, which nonemptiness never constrains.
Accordingly $\prod_{i=1}^{17}p_i$ falls with $r+s\ge34$: it is $r+s\ge34$ that supplies the $17$.
Two arithmetic slips sit in the same proposition: Table~2 lists $P_{11}=27$, and the displayed
``product of the first $17$ primes'' is printed with $27$ in place of $29$.

An explicit model is in \texttt{code/kovic\_prop18\_audit.gp}: with
$n=2\cdot5\cdot13\cdot37\cdot53\cdot73\cdot79\cdot89\cdot97\cdot101$ and
$m=3\cdot7\cdot19\cdot31\cdot43\cdot59\cdot67\cdot71\cdot83\cdot103$, squarefree with disjoint
supports and $r=s=10$, all four inequalities of (A) and (B) hold while $p_1q_s=206>100=rs$ and
$q_1p_r=303>100$. The valid products and the invalidity of the step are machine--checked in
\texttt{lean/Erdos307/KovicProp18.lean}.

What is refuted is the \emph{inference}, not the \emph{statement}. No two--cycle is known, so no
counterexample to $r+s\ge34$ can be exhibited and none is claimed; the conclusion is unproven rather
than false, and it is in fact true, since Theorem~\ref{thm:barrier} gives $|U|\ge60$ by a route that
touches none of this. Proposition~17, by contrast, is sound, and so is Proposition~16; but
Proposition~17 assumes the \emph{smaller} member is odd, and Kovi\v{c} observes that no estimate
follows when it is even, so it is a conditional bound. The consequence for the record is that the
strongest valid unconditional bound in the prior literature is the computer search $x>10^4$, that
Theorem~\ref{thm:barrier} improves it by $52$ orders of magnitude, and that $|U|\ge60$ is, as far as
we can determine, the first valid cardinality bound for arithmetic--derivative two--cycles rather
than an improvement on an earlier one. We note this without any suggestion that the gap diminishes
\cite{Kovic2012}, which remains the standard reference and whose Propositions~16 and~17 we use.
