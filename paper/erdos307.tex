...
barrier through a mechanism independent of the minus square.
...
region $r\ge1$ lies above $10^{112.9}$, so no computation can sample it
...
the prediction that solutions exist rests on a constant that no computation can estimate, even in
principle, because the barrier and the sampleable range are disjoint. The expected-count heuristic
for \#307 is therefore not merely unproved but uncalibrated.
...
So $a'$ is mass-poor for one reason only, that it is coprime to $a$ and $a$ has absorbed the small
primes, and there is no further effect to exploit. This is an independent confirmation of
Proposition~\ref{prop:nogain}: the barrier is not merely near-optimal as a bound, it already
contains the entire statistical structure of the obstruction, and no refinement by correlation is
available.
...
region: $0$ violations, least slack $0.242$ \textup{(\texttt{code/invent\_lyapunov.rs})}.
...
\begin{conjecture}\label{conj:lyap}
$(\ast)$ holds for some $\lambda>0$, uniformly over all squarefree $n$ with $n'$ squarefree.
\end{conjecture}
...
The lower demand admits a bound that is provable rather than measured, given one structural input.
Write $\rho=\sup\{\sigma(n')/\sigma(n):\sigma(n)>1\}$. If $\rho\le c<1$ then
$\sigma(n)-\sigma(n')\ge(1-c)\sigma(n)$, and since $\log x/x\le1/e$,
\[ L:=\sup_{\sigma(n)>1}\frac{\log\sigma(n)}{\sigma(n)-\sigma(n')}\;\le\;\frac1{e(1-c)} . \]
Measured, $\rho=0.519746$, attained at $n=177{,}769{,}130$ and stable across the range, which would
give $L\le0.766$ against a measured upper demand of $1.274$. So a proof of $\rho<1-1/(eU)$ for any
lower bound $U$ on the upper demand would establish Conjecture~\ref{conj:lyap}. That $\rho$ is
bounded away from $1$ is the coprimality mechanism of Proposition~\ref{prop:anticorr}: $\sigma(n)>1$
forces $n$ to absorb small primes, and $n'$ is coprime to $n$.

That mechanism is weakest when $n$ omits the small primes, and this was tested rather than assumed.
Every $n\le4\times10^8$ with $\sigma(n)>1$ is even, so the entire sweep measures $\rho$ on $n$
containing $2$; the odd case begins only at $3\cdot5\cdots29=3{,}234{,}846{,}615$. Constructing it
directly gives $\rho=0.0312$ there, and $\rho=0.0007$ for the least such $n$ with smallest prime
$5$, which has $27$ prime factors and $44$ digits. The regime that looked dangerous is in fact more
favourable, because reaching $\sigma(n)>1$ without small primes requires many of them, and $n'$ is
then a large generic integer of small mass.

In fact $\rho<1$ is provable in a range vastly exceeding any computation, by the barrier's own
mechanism.
...
The measured $\rho=0.519746$ therefore understates the situation: the quantity is not merely small
on the swept range but provably below $1$ for all $n$ up to $10^{56.5}$, against a sweep reaching
$4\times10^{8}$. The regime in which $\rho\ge1$ is possible is exactly the regime in which a
two-cycle is possible, since both require two coprime squarefree integers of total mass at least
$2$. That is the honest limit of the criterion: Conjecture~\ref{conj:lyap} cannot fail below the
barrier, and above it the criterion meets the same wall as everything else. It also means the
computational battery is superseded, since no feasible sweep approaches $10^{56.5}$.

\begin{remark}[What protects it, and what would kill it]\label{rem:lyapstatus}
The natural way to close the interval is to drive the upper demand to zero by taking
$\sigma(n)\uparrow1$, which makes $|\log\sigma(n)|$ vanish. That fails, and it fails because of
Proposition~\ref{prop:anticorr}: when $\sigma(n)$ is near $1$ the mass $\sigma(n')$ is small, so
those $n$ contribute \emph{lower} demands rather than upper ones. Quantitatively, among the $n$ with
$\sigma(n)\in[0.9,1)$ the largest $\sigma(n')$ is $0.548$, and this falls to $0.484$, $0.366$,
$0.366$ on the windows $[0.99,1)$, $[0.999,1)$, $[0.9999,1)$: the danger regime is empty, not merely
rare. The binding upper constraint comes instead from $\sigma(n)=0.2947$, far from $1$. The two demands are supported on
disjoint regimes, which is why the interval survives.

The status is \textup{HEURISTIC} and no more. A proof of Conjecture~\ref{conj:lyap} would answer
\#307, so the prior against it is severe, and the narrowing of the interval is exactly what one
would expect if it closes at some finite point. What is genuinely new is only that the negative
direction admits a falsifiable pointwise criterion at all: every previous no-go here concerned
existence of solutions, whereas $(\ast)$ is a statement about every $n$ separately and can be
refuted by a single example.
\end{remark}
