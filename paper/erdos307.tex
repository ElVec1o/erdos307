...
\cite{Kovic2012} \emph{on the odd member} of an $n''=n$ solution by roughly $47$ orders of
magnitude. The comparison is against that specific bound; measured against other bounds in the same
work the gain is smaller, and we make no claim about which of them is strongest.
\end{itemize}
...
character--sum evaluations as hypotheses; those evaluations are not in Mathlib, so this proposition
keeps its formalisation star.\textup{)}
...
contentless); Lemma~\ref{lem:symbolfact} is \texttt{Frame.csum\_eq\_mul\_recipSum} for its first half
...
\item \emph{What survives.} For a \emph{fixed} field the measurement stands: $\mathbb{Q}$ needs $59$
primes and $10^{112.9}$, whereas $\mathbb{Q}(i)$ needs $226$ prime ideals and $10^{735.1}$
\textup{(\texttt{code/a10\_fieldbarrier.gp})}. What does not survive is the use of this proposition
as a \emph{block}: the number--field route is not foreclosed by a barrier transfer. Whether an
ideal-theoretic solution in some $\mathcal{O}_K$ bears on \#307, which asks about rational primes,
is a separate question this paper does not settle. Status: \textsc{open}.
\end{enumerate}
\end{proposition}
...
Fix an odd squarefree $r>1$. \emph{The statement is qualitative and per fixed $r$; no uniformity in
$r$ is claimed, and the uniform form is false} \textup{(}Remark~\ref{rem:charcancelerratum}\textup{)}.
Then
\[ T_r(N)\;:=\!\!\sum_{\substack{m\le N,\ \mu^2(m)=1\\ \gcd(m,r)=1}}\!\!\Bigl(\frac{m'-2m}{r}\Bigr)\;=\;o_r(N)
\qquad (N\to\infty). \]
\end{lemma}
\begin{proof}
The argument runs at the modulus $r$ itself; no reduction to prime $r$ is needed, and the printed
proof's appeal to one is withdrawn. Four standard inputs are cited, all for \emph{fixed} $r$: Hal\'asz's theorem; the Gauss sum
evaluation $|\tau(\chi)|=\sqrt r$ for a primitive character; the bound
$|\log L(1+i\tau,\psi)|\ll_r\log\log(2+|\tau|)$ for $\psi$ non--principal; and Siegel--Walfisz, to
pass from $\log L$ to the partial sum $\sum_{p\le N}\psi(p)p^{-1-i\tau}$ over the tail. The third is
written with $r$ \emph{outside} the double logarithm deliberately. In $r$--uniform dress it is false:
the lower half needs $L(1,\psi)$ bounded away from $0$, and unconditionally one has only
$L(1,\psi)\gg r^{-1/2}$ effectively, or $\gg_\varepsilon r^{-\varepsilon}$ by Siegel and
ineffectively, so $|\log L(1,\psi)|$ can be of order $\log r$. At fixed $r$ these are fixed nonzero
constants and the argument is unaffected; the $r$--uniform form is exactly what
Corollary~\ref{cor:a9rate} would need and does not have.

Write $\chi=(\cdot\mid r)$. For $r$ odd squarefree the Jacobi symbol is a \emph{primitive} real
character modulo $r$, so $\chi(x)=\tau(\chi)^{-1}\sum_{t\bmod r}\chi(t)e_r(tx)$ for every $x$, with
$|\tau(\chi)|=\sqrt r$ \textup{(}checked numerically at $r=15,21,33,35,105,1155$\textup{)}. By
Lemma~\ref{lem:symbolfact}, $(m'-2m\mid r)=\chi(m)\,\chi(\sigma_r(m)-2)$ for squarefree $m$ coprime
to $r$, and expanding the second factor,
\[ T_r(N)=\tau(\chi)^{-1}\sum_{t\bmod r}\chi(t)\,e_r(-2t)\sum_{\substack{m\le N,\ \mu^2(m)=1\\ \gcd(m,r)=1}}h_t(m),
   \qquad h_t(m)=\chi(m)\!\!\prod_{p\mid m}\!e_r\bigl(t\,p^{-1}\bigr), \]
Extend $h_t$ to all of $\mathbb{N}$ by $h_t(p^k)=0$ for $k\ge2$ and $h_t(p)=0$ for $p\mid r$, the
only extension preserving the sum. It is then multiplicative with $|h_t|\le1$, which is Hal\'asz's
hypothesis; note $|h_t|=1$ holds on the support only, and the completely multiplicative extension
would \emph{not} preserve the identity.

It remains to show the Hal\'asz distance
$M=\min_{|\tau|\le\log N}\sum_{p\le N}\bigl(1-\operatorname{Re}h_t(p)p^{-i\tau}\bigr)/p$ tends to
infinity. Since $M$ is already a minimum, no free $\tau$ survives; what must be uniform over
$|\tau|\le\log N$ are the two inner bounds below. Note first that $h_t$ is \emph{not} a function of $m\bmod r$, since it is built from the
factorisation of $m$: at $r=7$, for instance, $h_1(2)\ne h_1(9)$ though $2\equiv9$. What is a
function of the residue is its restriction to primes, and that is all the expansion needs. Put
$g_t:(\mathbb{Z}/r)^\times\to\mathbb{C}$, $g_t(a)=\chi(a)e_r(ta^{-1})$, so that $h_t(p)=g_t(p\bmod r)$
for $p\nmid r$, with the convention $h_t=0$ at $p\mid r$. Expand $g_t$ in Dirichlet characters,
$g_t=\sum_\psi c_\psi\psi$. The principal
coefficient is
...
using $\chi(a^{-1})=\chi(a)$ and reindexing by the involution $a\mapsto ta^{-1}$; the sum vanishes
when $\gcd(t,r)>1$, which is redundant here since the factor $\chi(t)$ in the expansion above
already annihilates those $t$. \emph{No primitivity is used for this identity}: it holds for any
quadratic character on any finite abelian group, as the formalisation below makes explicit.
Primitivity is used twice, but elsewhere: for $|\tau(\chi)|=\sqrt r$, and for the Fourier inversion
at arguments $x$ with $\gcd(x,r)>1$.

\textup{Formalised.} This identity is the one new algebraic step of the repair, and it is the step
the previous proof got wrong, so it is in Lean: \texttt{Erdos307.sum\_char\_twist\_inv} proves
$\sum_a\chi(a)\psi(t a^{-1})=\chi(t)\sum_a\chi(a)\psi(a)$ for a quadratic character on a finite
abelian group and \emph{any} $\psi$, resting on \texttt{quadratic\_inv}
\textup{(}$\chi(g^{-1})=\chi(g)$, pure monoid algebra\textup{)} and
\texttt{twist\_inv\_involutive}. The vanishing at $\gcd(t,r)>1$ is \emph{not} formalised: a
previous revision claimed it was, citing a lemma whose hypotheses turned out to be contradictory and
which therefore proved nothing; it has been deleted rather than repaired, and the vanishing is
redundant in the main line anyway. With $\psi=e_r$ this says the left side is a \emph{Gauss} sum, not a Sali\'e sum, which is precisely
why no reduction to prime modulus is needed
\textup{(\texttt{lean/Erdos307/CharSum.lean})}. The three analytic inputs, Hal\'asz,
$|\tau(\chi)|=\sqrt r$, and the bound on $\log L(1+i\tau,\psi)$, are cited and not formalised. Then
...
$\tau$; the non--principal terms are $O_r(\log\log\log N)$ for $|\tau|\le\log N$. That passage deserves its
own sentence, because the obvious route to it fails. The cited bound governs the \emph{complete}
Euler logarithm $\log L$, while what is needed is the truncation $\sum_{p\le N}\psi(p)p^{-1-i\tau}$,
and partial summation against Siegel--Walfisz sheds a factor $1+|\tau|$ from differentiating
$u^{-1-i\tau}$, giving $O_r(\log N)$ in the stated range, which is worse than trivial. The step that
works is to move to the shifted point $s=1+1/\log N+i\tau$: the shift costs $O(1)$ since
$\sum_{p\le N}\log p/(p\log N)\ll1$, the tail $\sum_{p>N}\psi(p)p^{-s}$ is $o(1)$ because
$e^{-v/\log N}$ absorbs the $1+|\tau|$, and the classical zero--free region then gives
$|\log L(s,\psi)|\ll_r\log\log(2+|\tau|)$.

The coefficients themselves need a bound too, and it is not the $L$--functions that supply it:
$|g_t|=1$ on $(\mathbb{Z}/r)^\times$ gives $\sum_\psi|c_\psi|^2=1$ by Parseval, so
$\sum_{\psi\ne\psi_0}|c_\psi|\le\sqrt{\varphi(r)}$ by Cauchy--Schwarz. Their smallness comes
entirely from the $L$--functions having no pole, \emph{not} from their coefficients: at $r=15$ four
of the eight $c_\psi$ tie the principal one at $\sqrt r/\varphi(r)=0.4841$, and at $r=21$ five of
twelve tie at $0.3819$, the minimum $1/\varphi(r)$ being attained only at $\psi=\chi$, where
$\chi\psi$ is principal and the sum degenerates to a Ramanujan sum. Hence
\[ M\;\ge\;\Bigl(1-\frac{\sqrt r}{\varphi(r)}\Bigr)\log\log N-O_r(\log\log\log N), \]
and $\sqrt r/\varphi(r)\le\sqrt3/2=0.8660\ldots$ for every odd squarefree $r>1$, the maximum being
at $r=3$. That is not only a sweep: each local factor is $\sqrt p/(p-1)$, and
$(p-1)/\sqrt p\ge2/\sqrt3$ for every odd prime, so the supremum is the single factor at $r=3$
\textup{(\texttt{code/charcancel\_check.gp} corroborates to $r\le2\times10^5$)}.

The constant is moreover \emph{asymptotically exact}, not merely a valid lower bound: computing
$M$ directly at $r=5$, $t=1$ gives growth rate $\Delta M/\Delta\sum_{p}1/p=0.4411,0.4417,0.4419$ at
$X=10^4,10^5,10^6,10^7$ against $1-\sqrt5/\varphi(5)=0.440983$. Two caveats from the same
computation. At finite $X$ the measured $M$ sits \emph{below} the leading term, $1.201$ against
$1.341$ at $r=5$, $X=10^7$, which is the $-O_r(\log\log\log N)$ deficit and confirms the lemma is
asymptotic with no useful content at feasible $N$. And when $r\equiv3\pmod4$ the Gauss sum
$\tau(\chi)$ is purely imaginary, so the main term is imaginary, the bound is far from sharp, and
the true $M$ is of order $\log\log N$. So $M\gg\log\log N$. Note that $M\to\infty$ alone would not suffice: with the cut-off $T$ held
fixed Hal\'asz gives only $\ll N T^{-1/2}$. What delivers $o(N)$ is that $T=\log N\to\infty$ as
well, so both limits are in play. Then
Hal\'asz gives $S_t(N):=\sum_mh_t(m)\ll N\bigl((1+M)e^{-M}+(\log N)^{-1/2}\bigr)=o(N)$. Assembling,
and noting that only the $\varphi(r)$ values of $t$ coprime to $r$ contribute since $\chi(t)=0$
otherwise,
\[ |T_r(N)|\;\le\;r^{-1/2}\!\!\sum_{\gcd(t,r)=1}\!\!|S_t(N)|\;\le\;\frac{\varphi(r)}{\sqrt r}\,
   \max_t|S_t(N)|\;\le\;\sqrt r\,\max_t|S_t(N)|\;=\;o_r(N). \]
The net factor is a \emph{loss} of $\sqrt r$, not a gain of $r^{-1/2}$: the $\varphi(r)$ summands
outweigh the $|\tau(\chi)|^{-1}$. At fixed $r$ that is a constant and the lemma follows, but the
factor is not decorative, and it is the same $\sqrt r$ that reappears in
Corollary~\ref{cor:a9rate} as $T_r(N)\ll\sqrt r\,N(\log N)^{-1/2}$ and forces the exponent $1/4$
there.
\end{proof}

\begin{remark}[Erratum: the $\tau$ bound above is false near the threshold]\label{rem:charcancelerratum}
The step ``if $|\tau|\ge1/\log N$ then
$\bigl|\sum_{p\le N}p^{-1-i\tau}\bigr|\ll\log\log(3+|\tau|)$'' is \emph{false} in the range that
matters. Near the threshold the sum has size about $\log(1/|\tau|)$, which is of order $\log\log N$,
...
Consequences, stated exactly. Neither defect touches the lemma as displayed, because the proof above
does not use the offending step: it bounds the \emph{twisted} sum through the character expansion of
$h_t$, where the principal coefficient contributes $\sqrt r/\varphi(r)\le0.8661$ of
$\sum_{p\le N}1/p$ and the non--principal ones contribute $O_r(\log\log\log N)$, both uniformly in
$\tau$. The trivial bound $\bigl|\sum_{p\le N}p^{-1-i\tau}\bigr|\le\sum_{p\le N}1/p$ is all that is
needed of the untwisted sum, so the false inequality is not merely repaired but unnecessary.

What does \emph{not} survive is uniformity in $r$: the error above is $O_r(\log\log\log N)$, with the
implied constant depending on $r$ through the non--principal $L$--functions. Corollary~\ref{cor:a9rate}
needs a bound uniform in $r=pq$ as $P\to\infty$, which this does not supply, and it is independently
blocked by the regime mismatch recorded after Lemma~\ref{lem:reversal}. That corollary therefore keeps
the label \textsc{conjecture}.
\end{remark}
...
than in Lemma~\ref{lem:charcancel}, not by the same computation. Hence the
...
\textup{Label restored.} A previous revision downgraded this to \textsc{conjecture}, because the
square sieve below consumes Lemma~\ref{lem:charcancel} at the \emph{composite} moduli $r=pq$ and that
lemma's printed proof reduced composite $r$ to prime $r$ by assertion. The reduction is now
unnecessary: the corrected proof of Lemma~\ref{lem:charcancel} runs at the modulus $r$ itself, for
every odd squarefree $r>1$ at once, so the gap this theorem inherited is closed and the label returns
to \textsc{proved}. It is not \textsc{verified}: none of this is in Lean.

$\#\{m\le N:\ \mu^2(m)=1,\ m'-2m\ \text{is a positive perfect square}\}=o(N)$. Equivalently, atom
$\mathrm{A}9$ holds on the squarefree stratum: the squarefree minus layer has density zero. The same
argument with $m'+2m$ reproves Proposition~\textup{\ref{prop:plusthin}} there, so the squarefree
Pythagorean layer has density zero unconditionally; that proposition also has its own elementary
proof through the divisor sum, independent of this one.

The squarefree restriction is not cosmetic and earlier versions of this paper omitted it. Every tool
...
$|E_{pq}(N)|\le\#\{m\le N:p\mid m\text{ or }q\mid m\}\le N(1/p+1/q)\le2N/P$, so
...
pairs, by Lemma~\ref{lem:charcancel}. A square divisible by $p$ is divisible by $p^2$, so the excluded $m$
...
\emph{That uniformity is exactly what is not available}, and earlier editions of this paragraph
asserted the opposite. Lemma~\ref{lem:charcancel} gives
$M\ge\bigl(1-\sqrt r/\varphi(r)\bigr)\log\log N-O_r(\log\log\log N)$, with the error depending on
$r$ through the non--principal $L$--functions, and its statement and the erratum following it both
say so. The constant is $\sqrt r/\varphi(r)$, not the $3/\sqrt r$ printed here previously; the two
are not interchangeable, since $\varphi(r)\ge r/3$ fails for odd squarefree $r$, first at
$r=3\cdot5\cdots23=111{,}546{,}435$ with $\varphi(r)/r=0.3272$. The corollary below is therefore
stated as a \textsc{conjecture} and its argument is a sketch of what a uniform version would give,
not a proof.

\begin{corollary}[A rate for $\mathrm{A}9$: \textsc{conjecture}]\label{cor:a9rate}
The following rate does \emph{not} follow from Lemma~\ref{lem:charcancel}, which is qualitative and
per fixed $r$; deducing it needs a bound uniform in $r=pq$ as $P\to\infty$, and that is not
established here. It is independently blocked by the regime mismatch recorded after
Lemma~\ref{lem:reversal}. The statement is retained as a conjecture, nothing in this paper depends
on it, and what follows is a sketch of the balancing a uniform version would permit rather than a
proof.
...
\emph{Regime caveat.} That $O(1)$ holds only at $p\asymp N^{1/2}$. The application in
Corollary~\ref{cor:a9rate} balances at $P\asymp(\log N)^{1/4}$, where the same estimate gives
$N/p^2\asymp N/(\log N)^{1/2}$, which is not $O(1)$ and is not $o(N)$ either. The two regimes are
disjoint, so this lemma does not supply the error control that corollary needs. This is a second,
independent reason for the label \textsc{conjecture} carried there.
\end{lemma}
\begin{proof}
