...
proof's appeal to one is withdrawn. Three standard inputs are cited: Hal\'asz's theorem, the Gauss
sum evaluation $|\tau(\chi)|=\sqrt r$ for a primitive character, and
$|\log L(1+i\tau,\psi)|\ll\log\log\bigl(r(2+|\tau|)\bigr)$ for $\psi$ non--principal.
...
infinity, and here the key point is that the estimate is \emph{uniform in $\tau$}. Expand
$h_t$ on $(\mathbb{Z}/r)^\times$ in Dirichlet characters, $h_t=\sum_\psi c_\psi\psi$. The principal
coefficient is
...
using $\chi(a^{-1})=\chi(a)$ and the primitivity above; the sum vanishes when $\gcd(t,r)>1$.
...
\texttt{twist\_inv\_involutive}; \texttt{sum\_char\_twist\_inv\_eq\_zero} is the vanishing case.
With $\psi=e_r$ this says the left side is a \emph{Gauss} sum, not a Sali\'e sum, which is precisely
...
$\tau$; the non--principal terms are $O_r\bigl(\log\log(r(2+|\tau|))\bigr)=O_r(\log\log\log N)$ for
$|\tau|\le\log N$. Hence
