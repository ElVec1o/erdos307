...
$P=(\log\log N)^{1-\delta}$, where two uniform estimates \emph{are} available ---
...
$N\log\log\log N/(\log\log N)^{1-\delta}$, is very weak; the far stronger
Corollary~\ref{cor:a9rate} remains conjectural and independently blocked. Nothing else in this paper
...
The argument recorded in this section is a square sieve whose analytic input does not close, which
...
Proposition~\ref{prop:condrate}, at the much smaller $P=(\log\log N)^{1-\delta}$, where the same
...
\emph{range--dependent}. At $r\le(\log\log N)^{2-\delta}$ one has $\log r\asymp\log\log\log\log N$,
so even a Siegel--worst--case $|\log L(1,\psi)|$ costs $\varepsilon\log r+O_\varepsilon(1)$, which is
negligible against the main term $\log\log N$; that is Lemma~\ref{lem:charcancelunif}, and with
Lemma~\ref{lem:deficit} it proves Theorem~\ref{thm:a9} with a rate. What the erratum forecloses is
uniformity at the far larger $r$ that Corollary~\ref{cor:a9rate} needs, where $\log r$ is comparable
to the main term; that corollary is also independently blocked by the regime mismatch recorded after
Lemma~\ref{lem:reversal}, so it keeps the label \textsc{conjecture}.
\end{remark}
...
with an implied constant that depends only on $\delta$ and is ineffective; the same bound, by the
...
\textup{(Label discipline: written after Lemma~\ref{lem:charcancelunif} earned its label, and
subject to the same standard --- it carries no label until a review that did not write it finds
nothing.)}
Let $\delta\in(0,1)$ and $c\in\{2,-2\}$. Uniformly for primes $p\ne q$ in
$(\tfrac12(\log\log N)^{1-\delta},(\log\log N)^{1-\delta}]$, with implied constants that depend only
on $\delta$ and are ineffective,
...
\end{lemma}
\begin{proof}
Write $L=\log\log N$ and $P=L^{1-\delta}$, so $p,q\in(P/2,P]$ and $pq\le P^2=L^{2-2\delta}$. Recall
...
$\sum_{\psi\ne\psi_0}|c_\psi|\le\sqrt{\varphi(pq)}\le\sqrt{pq}\le L^{1-\delta}$.

\emph{The non--principal sums.} This is verbatim the corresponding step of
Lemma~\ref{lem:charcancelunif}, at modulus $pq\le L^{2-2\delta}$ in place of $r\le L^{2-\delta}$:
...
$|s-\beta|>1-\beta\ge c(\varepsilon)(pq)^{-\varepsilon}$ and hence
$|\log(s-\beta)|\le\varepsilon\log(pq)+O_\varepsilon(1)\ll\varepsilon\log L+O_\varepsilon(1)$, and
...
\[ M\;=\;\min_{|\tau|\le\log N}\sum_{\ell\le N}\frac{1-\operatorname{Re}h_{s,t}(\ell)\ell^{-i\tau}}{\ell}
   \;\ge\;\bigl(1-o(1)\bigr)L\;-\;O\bigl(L^{1-\delta}\log L\bigr)\;=\;\bigl(1-o(1)\bigr)L, \]
the principal part costing at most $|c_{\psi_0}|L=o(L)$ and the non--principal part
$O(L^{1-\delta}\log L)=o(L)$. Hal\'asz's theorem (Montgomery's form; absolute constants, and
...
$pq\le L^{2-2\delta}$, so $pq=o\bigl((\log N)^{1/2}\bigr)$ and hence
...
principal coefficient $-1/(p-1)$, $\sum_{\psi\ne\psi_0}|c_\psi|\le\sqrt p\le L^{(1-\delta)/2}$, and
...
\textup{Label: \textsc{proved}, and by a different route than the one that carried this statement
at \textsc{conjecture} through several releases. That earlier argument diagonalised over $r$ and
needed a $\#\{p^2\mid a_m\}$ input; its mathematics survived four rounds of review but its
\emph{text} failed three, a load-bearing display silently shedding a hypothesis each time. It is
superseded, not repaired: Proposition~\textup{\ref{prop:condrate}} proves the statement with a rate,
using no diagonalisation and no local square count, from
Lemma~\textup{\ref{lem:charcancelunif}} and Lemma~\textup{\ref{lem:deficit}}, each of which earned
its own label under the same standard --- a review that did not write it finding nothing.}

$\#\{m\le N:\ \mu^2(m)=1,\ m'-2m\ \text{is a positive perfect square}\}=o(N)$; indeed, by
Proposition~\ref{prop:condrate}, the count is
$\ll_\delta N\log\log\log N/(\log\log N)^{1-\delta}$ for every $\delta\in(0,1)$. Equivalently, atom
...
Proposition~\ref{prop:condrate}, and remains conjectural. The uniformity in $r$ that it needs is now
available --- that is Lemma~\ref{lem:charcancelunif} --- but only for $r\le(\log\log N)^{2-\delta}$,
whereas the balancing below sits at $P\asymp(\log N)^{1/4}$, incomparably larger; so the new lemma
does not reach it. The second blocker is untouched: Lemma~\ref{lem:localsq} at $P=P(N)$ would have to
be uniform in $p$ and is not, its $o(1)$ being driven by non--principal $L$--functions modulo $p^2$
and ineffective in $p$. Theorem~\ref{thm:condpower} meets the same obstruction and sidesteps it with
a divisor bound instead. Nothing in this paper depends on the statement, and what follows is a sketch
of the balancing a uniform version at that range would permit rather than a proof.
...
Proposition~\ref{prop:condrate}'s $N\log\log\log N/(\log\log N)^{1-\delta}$; Corollary~\ref{cor:a9rate}
