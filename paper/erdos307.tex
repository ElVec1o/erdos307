...
step rather than an arithmetic obstruction. Whether it can be avoided --- by applying
Siegel--Walfisz to the residue classes directly, paying a factor $r$ against an error
$\exp(-c\sqrt{\log t})$ rather than $\sqrt r$ against a bound on $\log L$ --- is open here, and is
the single question whose answer would most improve everything in this section.
\end{remark}
...
\emph{What does block it.} One thing only, and it is the pretentious distance --- in particular
\emph{not} the deficit inputs, which is worth recording since Lemma~\ref{lem:deficit} is stated only
for $p\asymp\log\log N$ and its closing step fails at exactly this $P$. That failure is irrelevant:
what the sieve needs here is not $\#\{m:p\mid a_m\}\ll N/p$ but only $\ll N/\Pi\asymp N\log P/P$,
weaker by the whole factor $\log P$; and the two--prime count needs nothing at all, since
$\#\{m:p\mid a_m,\,q\mid a_m\}\le\#\{m:p\mid a_m\}$ makes
$\sum_{p\ne q}\#\{\cdots\}\le\Pi\sum_p\#\{\cdots\}\ll\Pi N$ automatically. The detector also costs
nothing extra here: its $1/(pq)$ prefactor exactly cancels the $pq-1$ non--principal terms, with no
$\sqrt r$ amplification of the kind $|T_r|\le\sqrt r\max_t|S_t|$ carries --- which is why the deficit
side is the easier of the two. At
...
is of order $\sqrt r$, and Cauchy--Schwarz is losing nothing. Nor would a better $L$--function input help \emph{within this route}: an $O(1)$
per--character bound still leaves $\sqrt{\varphi(r)}=o(\log\log N)$, hence $P=o(\log\log N)$, short of
$(\log N)^{1/4}$ by the same exponential. Whether the route itself can be replaced --- estimating the
distance without expanding at all --- is the open question of Remark~\ref{rem:a9ceiling}, and we do
not assert that it cannot be. Nothing in this paper depends on the statement, and what
