...
$\ll N\log\log N(\log N)^{-1/4}$ needs uniformity at a much larger range and is stated as a
\textsc{conjecture}, not proved. The
...
Corollary~\ref{cor:a9rate} remains conjectural, blocked by one thing only --- the pretentious
distance at moduli $r\le(\log N)^{1/2}$, where the coefficient mass $\sqrt{\varphi(r)}$ overwhelms
$\log\log N$; two blockers recorded for it in earlier revisions are retracted in
Remark~\ref{rem:a9ceiling} and at the corollary. Nothing else in this paper
...
input does not close, which is why the \emph{rate} Corollary~\ref{cor:a9rate} stands at
\textsc{conjecture} and why Remark~\ref{rem:whatisleft} keeps this material off the critical path.
...
revisions read that as a second, independent reason for the \textsc{conjecture} label on
Corollary~\ref{cor:a9rate}. It is not, for two reasons. The comparison there was made against $N$,
whereas the benchmark is that corollary's own target $N\log\log N(\log N)^{-1/4}$, against which
$N(\log N)^{-1/2}$ is \emph{smaller} --- an error term below the claimed rate cannot obstruct it.
And the corollary does not travel this route at all: re--derived as in
...
gap is what identified the workable range, and because Corollary~\ref{cor:a9rate}, still
conjectural, is what this route would give if its input were ever supplied.
...
moduli of Corollary~\ref{cor:a9rate} the $L$--function bound does also fail, but it is not what
...
$L$--function; see the blocker recorded at that corollary. What the erratum forecloses is
uniformity at the far larger $r$ that Corollary~\ref{cor:a9rate} needs, where $\log r$ is comparable
to the main term. That corollary keeps the label \textsc{conjecture}, but for the single reason
recorded there, which is neither this one nor the regime mismatch after
Lemma~\ref{lem:reversal}: both of those were retracted.
\end{remark}
...
Let $\varepsilon>0$, $c\in\{2,-2\}$, $L=\log\log N$ and $P=L/(\log L)^{1+\varepsilon}$. Uniformly
for primes $p\ne q$ in $(\tfrac12P,P]$, with implied constants that depend only on $\varepsilon$ and
are ineffective,
...
\end{lemma}
\begin{proof}
Here $p,q\in(P/2,P]$ and $pq\le P^2=L^2/(\log L)^{2+2\varepsilon}$, inside
...
$\sum_{\psi\ne\psi_0}|c_\psi|\le\sqrt{\varphi(pq)}\le\sqrt{pq}\le L/(\log L)^{1+\varepsilon}$.

\emph{The non--principal sums.} This is verbatim the corresponding step of
Lemma~\ref{lem:charcancelunif}, at modulus $pq\le L^2/(\log L)^{2+2\varepsilon}$:
...
$\varepsilon$ of the range, and hence
...
\[ M\;=\;\min_{|\tau|\le\log N}\sum_{\ell\le N}\frac{1-\operatorname{Re}h_{s,t}(\ell)\ell^{-i\tau}}{\ell}
   \;\ge\;\bigl(1-o(1)\bigr)L\;-\;O\bigl(L(\log L)^{-\varepsilon}\bigr)\;=\;\bigl(1-o(1)\bigr)L, \]
the principal part costing at most $|c_{\psi_0}|L=o(L)$ and the non--principal part
$O\bigl(L(\log L)^{-\varepsilon}\bigr)=o(L)$. Hal\'asz's theorem (Montgomery's form; absolute constants, and
...
\[ \#\{m\le N:\ \mu^2(m)=1,\ m'-2m\ \text{a positive perfect square}\}
   \;\ll_\varepsilon\;N\,\frac{(\log\log\log N)^{2+\varepsilon}}{\log\log N} , \]
and the same for $m'+2m$. In particular Theorem~\ref{thm:a9} follows with a rate, by an
argument with no diagonalisation over $r$.
\end{proposition}
\begin{proof}
Let $L=\log\log N$, $P=L/(\log L)^{1+\varepsilon}$, $\mathcal P$ the primes in $(P/2,P]$, $\Pi=|\mathcal P|\gg
...
Lemma~\ref{lem:charcancelunif} at $r=pq\le L^2/(\log L)^{2+2\varepsilon}$ (each $r$ odd squarefree,
...
bound follows, $\log P\asymp\log L$ giving the numerator $(\log L)^{2+\varepsilon}$ after the
$P$ in the denominator is written out. Note $\mu^2(m)$ and the coprimality conditions are carried inside every count;
...
matters is the shape: \emph{no} choice of $P$ puts a power of $\log N$ in the denominator, so this
method cannot reach Corollary~\ref{cor:a9rate} however it is tuned.

\emph{The one input that would.} Both walls --- this one and the corollary's --- are the same
$\sqrt{\varphi(r)}$ paid when the phase $\ell\mapsto e_r(t\ell^{-1})$ is expanded in multiplicative
characters. A direct estimate would remove it outright:
\[ \sum_{\ell\le X}e_p(s\ell^{-1})\,\ell^{-1-i\tau}\;=\;o(\log\log X), \qquad
   \text{uniformly for } s\not\equiv0\ (p),\ |\tau|\le\log X,\ p\le(\log X)^{A}. \]
This is a Weil--type bound for a phase over \emph{primes} whose argument is an inverse rather than a
polynomial. With it, $M\ge(1-o(1))\log\log N$ holds at $p\asymp(\log N)^{1/4}$ and
Corollary~\ref{cor:a9rate} follows as stated. It is the additive form of the same deficit
Remark~\ref{rem:Ameasured} records for Hypothesis~\ref{hyp:sq}, and stating it as one question rather than
two is the only consolidation this section offers.
\end{remark}

\begin{corollary}[A rate for $\mathrm{A}9$: \textsc{conjecture}]\label{cor:a9rate}
This rate is far stronger than the unconditional
Proposition~\ref{prop:condrate} and remains conjectural, but the reason has been restated: two
earlier revisions recorded two blockers, and neither was the operative one.

\emph{What does not block it.} It was said to be blocked by Lemma~\ref{lem:localsq}, non--uniform in
$p$. That is true of the sketch below, which inherits its $p^2$--count from
Remark~\ref{rem:a9original}; it is not true of the corollary. Re--derived along the lines of
Proposition~\ref{prop:condrate} --- which bounds $\sum_p(a_m\mid p)\ge\Pi-\#\{p\mid a_m\}$ and so
needs only \emph{first}--power divisibility --- Lemma~\ref{lem:localsq} never enters;
Theorem~\ref{thm:condpower} avoids it a third way, by a divisor bound. It was also said to be
blocked by the regime mismatch after Lemma~\ref{lem:reversal}. That belongs to the kernel
reformulation, which this corollary does not use --- and which Proposition~\ref{prop:circular} shows
is no reduction at all --- and in any case that mismatch's error $N(\log N)^{-1/2}$ is
\emph{smaller} than the rate claimed here, so it could not block it: the comparison there was made
against $N$, which no term in a rate argument need beat.

\emph{What does block it.} One thing only, and it is the pretentious distance. At
$P\asymp(\log N)^{1/4}$ every other term clears from the bare Hal\'asz floor $N(\log N)^{-1/2}$,
which is already smaller than the target $N\log\log N(\log N)^{-1/4}$. What is required is
$M\ge(\tfrac12+\delta)\log\log N$, uniformly for $r\le(\log N)^{1/2}$ and $|\tau|\le\log N$: the
sketch below carries the expansion loss $\sqrt r\le2P$, so Hal\'asz must supply
$\sqrt r\,e^{-M}\lesssim(\log N)^{-1/4}$ and not merely $e^{-M}\lesssim(\log N)^{-1/4}$ --- and that
factor is the very reason $P$ balances at $(\log N)^{1/4}$, so it cannot be dropped. What is
available is $M\ge\log\log N-\sqrt{\varphi(r)}\cdot O(\log\log\log N)$, vacuous once
$\sqrt r\log\log\log N\gtrsim\log\log N$. The shortfall, measured by $\sqrt r\log L/L$ with
$L=\log\log N$, is
$1.12$, $6.24$, $130.5$, $1.9\times10^6$ at $\log\log N=5,14,28,69$
\textup{(\texttt{code/wall\_check.gp})}. And it is not an artefact of Cauchy--Schwarz. For prime
$r$, $|c_\psi|=\sqrt{\operatorname{cond}(\chi\psi)}/(r-1)$, so all but one of the $r-1$ coefficients
equal $\sqrt r/(r-1)$; the exception is $\psi=\chi$, where $\chi\psi=\chi_0$ has conductor $1$ and
$|c_\psi|=1/(r-1)$. Hence
$\sum_{\psi\ne\psi_0}|c_\psi|=\bigl((r-3)\sqrt r+1\bigr)/(r-1)=\sqrt r\,(1+O(1/r))$: the mass really
is of order $\sqrt r$, and Cauchy--Schwarz is losing nothing. Nor would a better $L$--function input help --- an $O(1)$ per--character bound
still leaves $\sqrt{\varphi(r)}=o(\log\log N)$, hence $P=o(\log\log N)$, short of
$(\log N)^{1/4}$ by the same exponential. Nothing in this paper depends on the statement, and what
follows is the original sketch rather than a proof.
...
Proposition~\ref{prop:condrate}'s $N(\log\log\log N)^{2+\varepsilon}/\log\log N$; Corollary~\ref{cor:a9rate}
would be far stronger and remains conjectural.
\end{remark}
