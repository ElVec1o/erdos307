...
This is $1.6\times10^{5}$ times closer to $1$ than the preceding record. The construction is not
exhausted: over the same sweep the smallest \emph{available} overshoot was $1.209\times10^{-14}$, and
what limits the reported value is the rate at which $a'$ takes the form $2\cdot3\cdot5\cdot P$, not the
tuning \textup{(\texttt{code/nearmiss\_ladder.gp}, re--checked in
\texttt{code/nearmiss\_ladder\_verify.gp})}. Consistently with the two--sided statement above, $a$
