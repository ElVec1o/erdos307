...
\textup{(Label discipline, as for Theorem~\ref{thm:a9}: this proof has been through three rounds of
adversarial review, each by a session that did not write it; each found defects, which were
repaired, so under the project's standard the statement does not yet carry \textsc{proved},
which requires a round that finds nothing. Proposition~\ref{prop:condrate} passed round three
clean.)}
Fix $\delta\in(0,1)$. For every odd squarefree $r$ with $7\le r\le(\log\log N)^{2-\delta}$,
\[ T_r(N)\;\ll\;N\,(\log N)^{-2/5}, \]
with an implied constant that depends only on $\delta$ and is ineffective; the same bound, by the same proof with
