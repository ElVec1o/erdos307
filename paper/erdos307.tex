...
$\ll N\log\log\log N(\log\log N)^{-1+\delta}$ for every $\delta>0$, which rests on a cancellation
estimate uniform for moduli up to $(\log\log N)^{2-\delta}$; the far stronger
...
$\ll_\delta N\log\log\log N/(\log\log N)^{1-\delta}=o(N)$ on the squarefree stratum for any fixed
$\delta\in(0,1)$, and the plus--layer count by the same statement. Remark~\ref{rem:a9original}
