...
with an implied constant that is absolute but ineffective. For $r\in\{3,5\}$ the same holds with
exponents $c_3-\varepsilon=0.133\ldots$ and $c_5-\varepsilon=0.441\ldots$ in place of $2/5$, as in
...
put $s=1+1/\log N+i\tau$. Partial summation against Siegel--Walfisz, exactly as in
...
$\psi$ is real with an exceptional zero $\beta$, then
$\log L(s,\psi)=\log(s-\beta)+O\bigl(\log\log(r(3+|\tau|))\bigr)$, and for \emph{every}
$|\tau|\le\log N$, not only $\tau=0$,
\[ |s-\beta|\;\ge\;\operatorname{Re}(s)-\beta\;>\;1-\beta\;\ge\;c(\varepsilon)\,r^{-\varepsilon} \]
by Siegel's theorem in zero--location form, so
$|\log(s-\beta)|\le\varepsilon\log r+O_\varepsilon(1)\ll\varepsilon\log L+O_\varepsilon(1)$. In
both cases $|S_\psi(\tau)|\ll\log L$, absolutely and ineffectively, uniformly in
$|\tau|\le\log N$.
...
factor satisfies $(p-1)/\sqrt p\ge2/\sqrt3$ and only $r=3,5$ fall below
\textup{(\texttt{code/uniform\_constants\_check.gp})}. Hal\'asz
...
Uniformly for primes $p\ne q$ in $(\tfrac12(\log\log N)^{1-\delta},(\log\log N)^{1-\delta}]$,
with absolute implied constants,
\[ \#\{m\le N:\ \mu^2(m)=1,\ \gcd(m,p)=1,\ \sigma_p(m)\equiv2\ (p)\}\ \ll\ N/p, \qquad
   \#\{m\le N:\ \ldots\equiv2\ (p)\ \text{and}\ (q)\}\ \ll\ N/(pq). \]
...
$\sum_{p\in\mathcal P}(a_m\mid p)\ \ge\ \Pi-\#\{p\in\mathcal P:p\mid a_m\}$. Squaring and summing
over all squarefree $m\le N$,
\[ \Pi^2\,\#\{\text{squares}\}\ \le\ \sum_{p\ne q}\bigl(T_{pq}(N)+E_{pq}\bigr)\;+\;\Pi N
   \;+\;2\Pi\sum_{p}\#\{m:p\mid a_m\}\;+\;\sum_{p\ne q}\#\{m:\ p\mid a_m,\ q\mid a_m\}
   \;+\;\sum_p\#\{m:p\mid a_m\}, \]
where $E_{pq}\le2N/P$ collects the $m$ with $\gcd(m,pq)>1$, dropped from $T_{pq}$'s summation
...
$r\ge7$), $\sum_{p\ne q}|T_{pq}|\le\Pi^2N(\log N)^{-2/5}$. By Conjecture~\ref{conj:deficit},
