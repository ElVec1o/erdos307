...
solutions by about $47$ orders of magnitude. We add a parity dichotomy (a hypothetical all--odd
...
\emph{not} a feasible finite search. The rigidity result is fully formalised in Lean~4 (mathlib); the barrier is
formalised modulo one explicit, standard extremality hypothesis (that the $k$ smallest primes
minimise $\prod$ and maximise $\sum1/p$). The problem remains open; the heuristics weakly favour
existence.
\end{abstract}
...
rigidity result is fully Lean--verified; the barrier is Lean--verified modulo one explicit
extremality hypothesis. The heuristics weakly favour \textsc{yes} (existence), hence weakly against
the period--one Ufnarovski--\AA hlander conjecture, but we claim no proof in either direction.

Open problems: (i) prove the smallest--primes extremality and complete the Lean barrier; (ii) decide
