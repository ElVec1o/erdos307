...
\emph{range--dependent}. At $r\le(\log\log N)^2$ one has $\log r\asymp\log\log\log\log N$,
...
available --- that is Lemma~\ref{lem:charcancelunif} --- but only for
$r\le(\log\log N)^2/(\log\log\log N)^{2+\varepsilon}$,
