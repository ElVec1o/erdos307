...
\item \emph{Exporting} to that literature: our barrier (Theorem~\ref{thm:barrier}) improves the
known bounds on an $n''=n$ solution. Kovi\v{c} \cite{Kovic2012} proves two, and we compare against
the stronger. His Proposition~17 gives $>3\cdot5\cdots29\approx3.23\times10^9$ on the \emph{odd}
member; his Proposition~18 gives $r+s\ge34$ and, for \emph{at least one} member, the bound
$\prod_{i=1}^{17}p_i\approx1.92\times10^{21}$. Against the latter, $\min(\prod P,\prod Q)\ge
2.09\times10^{56}$ is a gain of $35.0$ orders of magnitude, and it is a gain in kind as well as in
size: it bounds \emph{both} members, not one. On cardinality, Kovi\v{c}'s Proposition~18 is the
antecedent of our $|U|\ge60$: he obtains $r+s\ge34$ unconditionally, and $r+s\ge57$ under the extra
hypothesis $\min\{p_1,q_1\}\ge3$, which \#307 does not supply. Our $|U|\ge60$ is unconditional. The
two arguments are independent --- his runs through $p_1q_s<rs$, ours through $\sum_{p\in U}1/p>2$.
\end{itemize}
...
\end{enumerate}
Consequently every $n''=n$ solution has both members $>2.09\times10^{56}$, improving the strongest
bound of \cite{Kovic2012} (Proposition~18, $\max\{m,n\}\ge\prod_{i=1}^{17}p_i\approx1.92\times
10^{21}$) by $35.0$ orders of magnitude, and improving it from one member to both.
\end{theorem}
...
\paragraph{Kovi\v{c} (2012), Proposition~18 \cite{Kovic2012}.}
A third item, older than both and not noticed at drafting. Kovi\v{c} bounds the two--cycle system
$n'=m$, $m'=n$ from the inequalities $p_1q_s<rs$ and $q_1p_r<rs$, where $r=\omega(n)$, $s=\omega(m)$,
obtaining $r+s\ge34$ unconditionally, $r+s\ge57$ if $\min\{p_1,q_1\}\ge3$, $r+s\ge110$ if
$\min\{p_1,q_1\}\ge5$, and hence $\max\{m,n\}\ge\prod_{i=1}^{17}p_i\approx1.92\times10^{21}$. This is
the true antecedent of our Theorem~\ref{thm:barrier}, and it is stronger than the Proposition~17
bound $\approx3.23\times10^9$ that we had cited as the comparison point; the introduction now
compares against Proposition~18. Our contribution over it is threefold: $|U|\ge60$ unconditionally
rather than $34$ (and without his $\min\{p_1,q_1\}\ge3$, which \#307 does not supply); the product
bound on \emph{both} members rather than one; and $35.0$ orders of magnitude in that bound. The
arguments are independent: his is a two--sided size comparison between a squarefree number and its
derivative, ours is $\sum_{p\in U}1/p=s+t>2$ by AM--GM followed by a prime--counting minimality
argument. We claim no priority for the \emph{existence} of a cardinality barrier, only for its
value and its unconditionality.
