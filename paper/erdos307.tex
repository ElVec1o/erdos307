...
prime absorbs; a direct scan over all primes $\ell\le600$ (for several bases $W$) finds the pair
system \emph{locally soluble everywhere} --- so, unlike the single--tail family, the pair sector
admits \emph{no} congruence obstruction at all. It is a genuine two--parameter surface per base,
$\#307$ in miniature (Remark~\ref{rem:oneside}); reciprocity cannot touch it, and only a direct
both--squares search or a descent argument can.
\end{remark}
...
only through $N\bmod4$. The kill rate $31{,}219/49{,}961=0.62487$ then sits within $0.06\sigma$ of
$\bigl(\tfrac34+\tfrac12+\tfrac12+\tfrac34\bigr)/4=5/8$, the conditional $3/4$ being exactly the
correlation between the genus parity $s+\binom{t}{2}$ and $N\bmod8$ (recall $N=D_1+2D_1'$). That
correlation is real on the level--59 set but \emph{not} universal --- a random--subset proxy gives
${\approx}0.45/0.55$ --- so the $5/8$ is this genus character's shadow on the particular ensemble, and
yields no second certificate.
\end{remark}
...
Open problems: (i) close the residual $3{,}478$ single--tail families at level $60$ and find an
arity--two kill for the pair sector of Proposition~\ref{prop:sectors} (the canonical tail family
...
law of Remark~\ref{rem:fiveeighths} awaits explanation); (ii) decide
...
$\tau(M'+2M)$ --- a Titchmarsh--divisor--type sub--problem --- would close exactly this gap. We
record it as the analytic question this layer actually poses.
\end{remark}
