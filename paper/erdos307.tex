...
entry region has now been swept: over the $1{,}842$ squarefree $N_0\le2\times10^{12}$ with
$\sigma(N_0)\ge\tfrac32$, the form $Q_{N_0}$ represents $4N_0^2$ not once, so there is no live slot
and no ghost either \textup{(\texttt{code/live\_slot\_census.rs})}. The same program run without the
