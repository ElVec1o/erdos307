...
\begin{lemma}[Cancellation in the character sums; fixed $r$ only]\label{lem:charcancel}
Fix an odd squarefree $r>1$. \emph{The statement is qualitative and per fixed $r$; no uniformity in
$r$ is claimed, and the uniform form is false.} See the erratum following the proof. Then
\[ T_r(N)\;:=\!\!\sum_{\substack{m\le N,\ \mu^2(m)=1\\ \gcd(m,r)=1}}\!\!\Bigl(\frac{m'-2m}{r}\Bigr)\;=\;o_r(N)
\qquad (N\to\infty). \]
\end{lemma}
\begin{proof}
Take $r=q$ prime. \emph{The reduction to prime $r$ is incomplete.} Expanding
$(\cdot/r)=\prod_{q\mid r}(\cdot/q)$ turns $T_r$ into a sum of a product of characters to distinct
moduli, and ``running the argument in each factor'' is not a proof: the Hal\'asz input applies to a
single multiplicative function, and the product over $q\mid r$ must be treated as one function to
the modulus $r$, with its own distance $M$. Theorem~\ref{thm:a9} consumes exactly the composite
moduli $r=pq$, so this gap is in the range that is used. What follows proves the prime case. Let
...
Consequences, stated exactly. The lemma as displayed, qualitative and for \emph{fixed} $r$, is what
Theorem~\ref{thm:a9} consumes: there $P$ is fixed first, finitely many pairs $pq$ are involved, and
$N\to\infty$ makes each $T_{pq}(N)=o(N)$; letting $P\to\infty$ afterwards gives density zero with no
uniformity used. Theorem~\ref{thm:a9} is therefore unaffected. What does \emph{not} survive is the
uniform-in-$r$ form, and with it the rate of Corollary~\ref{cor:a9rate}, which is why that corollary
now carries the label \textsc{conjecture}.
\end{remark}
...
\begin{theorem}[The minus layer has density zero, on the squarefree stratum: \textsc{conjecture}]\label{thm:a9}
\textup{Label downgraded.} The proof below reduces to Lemma~\ref{lem:charcancel} at the
\emph{composite} moduli $r=pq$, and that lemma's reduction from composite $r$ to prime $r$ is a
one--line assertion rather than an argument, as the erratum after it now records. The gap therefore
sits precisely in the range this theorem uses, so the statement is retained as a conjecture. What is
proved is the square--sieve skeleton together with the prime--modulus case of
Lemma~\ref{lem:charcancel}.

Two things are unaffected. Proposition~\ref{prop:plusthin} does not depend on this theorem: it has
its own elementary proof through the divisor sum, and only the remark that ``the same argument
reproves it'' falls. And the barrier, which is what \#307 actually rests on, is independent of the
whole minus--layer analysis.
...
argument with $m'+2m$ would reprove Proposition~\textup{\ref{prop:plusthin}} there, and inherits the
same gap; that proposition's own elementary proof is unaffected.
