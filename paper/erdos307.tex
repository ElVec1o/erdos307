...
hypothesis that $F$ vanishes exactly on the diagonal, which is
Lemma~\ref{lem:sigmainj}, itself \textup{VERIFIED}.
...
\begin{theorem}[The crux target, on one unsigned hypothesis]\label{thm:half}
Assume Hypothesis~\textup{\ref{hyp:kernel}} and the quadratic large sieve of Heath-Brown in the form
\[ \sum_{r\le R}\nolimits^{\flat}\Bigl|\sum_{k\le K}\nolimits^{\flat}b_k\Bigl(\frac kr\Bigr)\Bigr|^2
   \ \ll_\varepsilon\ (RK)^{\varepsilon}(R+K)\sum_k|b_k|^2 , \]
the flats denoting odd squarefree summation. Then
\[ \#\{m\le N:\ m'-2m\ \text{is a positive perfect square}\}\ \ll\ N^{1/2+o(1)} , \]
which is exactly the bound $\mathrm{CRUX}\text{-}\mathrm{A}9$ asks for.
\end{theorem}
\begin{proof}
By Lemma~\ref{lem:reversal} and $K\ll N^{1+o(1)}$, the large sieve with $R\asymp P^2$ gives
$\sum_{r\le R}|T_r(N)|^2\ll N^{o(1)}(R+N)\,N$. Cauchy--Schwarz over the $\Pi^2$ pairs then yields
$\Pi^{-2}\sum_{p\neq q}|T_{pq}|\le\Pi^{-1}\bigl(\sum_r|T_r|^2\bigr)^{1/2}
\ll\Pi^{-1}N^{o(1)}\bigl((R+N)N\bigr)^{1/2}$. Taking $P=N^{1/2}$, so $R\asymp N$ and
$\Pi\asymp N^{1/2}/\log N$, both this and the term $N/\Pi$ of Theorem~\ref{thm:condpower} are
$N^{1/2+o(1)}$. The side condition $\Pi\ge2\log(2N)/\log P$ holds with room.
\end{proof}

\begin{remark}[Status]\label{rem:halfstatus}
Two things are assumed and neither is proved here. The large sieve is quoted from the literature and
its precise form should be checked against the source before the theorem is used. Hypothesis
\ref{hyp:kernel} is measured, not proved: at $N=6.08\times10^6$ over $6{,}079{,}290$ squarefree $m$
the sum $\sum_k|b_k|^2$ is $24{,}069{,}288$, a ratio of $3.959$ to $N$, with largest multiplicity
$1707$ \textup{(\texttt{code/sqsieve\_kernel.rs})}. The ratio being an absolute constant rather
than a growing quantity is what $N^{1+o(1)}$ requires.

This route is preferable to the bilinear one of Remark~\ref{rem:missingdecomp} on three counts: it
needs no decomposition of $\sum_{m\le N}$, its analytic input is a published theorem rather than a
missing one, and it reaches $N^{1/2+o(1)}$ rather than $N^{3/4+o(1)}$. What it needs is a single
unsigned pair count. The unconditional record proved here remains
Corollary~\ref{cor:a9rate}.
\end{remark}
