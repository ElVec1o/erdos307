...
uniformity is claimed. Remark~\ref{rem:charcancelerratum} records why the uniform version is not
available and what was previously claimed here in error; Remark~\ref{rem:charcancelsharp}
records what the constant is and is not.
\end{lemma}
\begin{proof}
Five standard facts are used, all at fixed $r$: Hal\'asz's theorem; $|\tau(\chi)|=\sqrt r$ for a
primitive character; the classical zero--free region; $|\log L(s,\psi)|\ll_r\log\log(2+|\tau|)$ for
...
individually, three of the eight tying the principal one at $r=15$. Their contribution is small
because the $L$--functions have no pole, and reaching that needs care: the cited bound governs
$\log L$, an Euler product over all $p$, while what is required is the truncation. Partial summation
against Siegel--Walfisz applied directly sheds a factor $1+|\tau|$, which is fatal in the range
$|\tau|\le\log N$. Instead move to $s=1+1/\log N+i\tau$. The shift costs $O(1)$ by Mertens, since
$\sum_{p\le N}\log p/(p\log N)\ll1$; and the tail $\sum_{p>N}\psi(p)p^{-s}$ is $o(1)$ because
Siegel--Walfisz contributes $e^{-c\sqrt v}$, which defeats the factor $1+|\tau|$ that partial
summation costs, the total being $\ll(\log N)^2e^{-c\sqrt{\log N}}$. The zero--free region then
gives $|\log L(s,\psi)|\ll_r\log\log(2+|\tau|)$, so the non--principal terms are $O_r(1)$ uniformly
for $|\tau|\le\log N$. Hence
\[ M\;\ge\;c_r\log\log N-O_r(1),\qquad c_r:=1-\frac{\sqrt r}{\varphi(r)}\;\ge\;1-\frac{\sqrt3}2
   \;=\;0.1339\ldots, \]
...
$S_t(N)\ll N\bigl((1+M)e^{-M}+(\log N)^{-1/2}\bigr)$. Note $M\to\infty$ alone would not suffice:
at fixed $T$ the bound is only $\ll NT^{-1/2}$. Both limits are in play, and
$S_t(N)\ll_rN(\log N)^{-c_r+o(1)}=o(N)$. Only the $\varphi(r)$ values of $t$ coprime to $r$
...
Three things about $c_r=1-\sqrt r/\varphi(r)$ are worth recording, because an earlier revision
...
the true $M$ has constant $1$ rather than $c_r$. Measured at $r=5$, $X=10^7$: $M=1.201$ for the two
residues with $\chi(t)=+1$, against $M=2.191$ for the other two. So the constant is attained for
$r\equiv1\pmod4$ and $\chi(t)=+1$, and is not sharp otherwise.

\emph{The error term is $O_r(1)$, not $O_r(\log\log\log N)$.} At $r=5$, $t=1$ the deficit between
$M$ and $c_r\sum_{p\le X}1/p$ is $0.1407,0.1407,0.1405,0.1405$ at $X=10^4,\dots,10^7$, flat to four
places, while $\log\log\log X$ rises by $28\%$ over the same range. The $\log\log\log$ enters only
through uniformity in $\tau$.

\emph{The rate that follows, and the one that does not.} Hal\'asz gives
$T_r(N)\ll_rN(\log N)^{-c_r+o(1)}$. Since $c_3=0.134$ and $c_5=0.441$ are below $\tfrac12$, this
does \emph{not} yield $T_r(N)\ll\sqrt r\,N(\log N)^{-1/2}$ at those moduli, and an earlier revision
asserted that it did. Quantitatively the lemma is asymptotic in the strong sense: at $r=5$, with the
measured constants and an implied Hal\'asz constant of $1$, the bound first beats the trivial
$|T_r(N)|\le N$ near $\log N\approx148$, that is $N\approx10^{64}$.
\end{remark}
...
$\sum_{p\le N}1/p$ and the non--principal ones contribute $O_r(1)$, both uniformly in $\tau$. The
...
Two claims must be distinguished, and an earlier revision ran them together. What is shown false
...
lemma being false; the uniform lemma is simply \emph{not established here}, which is a weaker and
accurate statement. Either way Corollary~\ref{cor:a9rate} needs a bound uniform in $r=pq$ as
$P\to\infty$ that this argument does not supply, and it is independently blocked by the regime
mismatch recorded after Lemma~\ref{lem:reversal}, so it keeps the label \textsc{conjecture}.
\end{remark}
