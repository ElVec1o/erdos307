...
\date{Version 1.65.0\quad 2 September 2026}
...
Level $60$ is decidable, and it is worth pricing the two routes, because they identify the obstruction
as a much smaller object than the residual. By Proposition~\ref{prop:tailbound} the tail prime is a
root of a quadratic indexed by a divisor $\alpha\mid D_S$, so a family is decided by $2^{58}$
perfect--square tests on integers of some $230$ digits: about $3.2\times10^7$ core--years over the
$3{,}478$, and about $3.1\times10^5$ over the $34$ immune ones alone. Factoring the balanced semiprime
instead costs on the order of twenty core--years for all $3{,}478$, which is feasible, but it is
incomplete by a theorem: for $A=pq$ with $(D_S\mid A)=+1$ either both Legendre symbols are $+1$, when
no kill exists, or both are $-1$, when one does, so factoring decides at best half; and the $34$
immune families carry no inert factor at all, so no congruence kills them however far the factoring is
pushed. The bottleneck at level $60$ is therefore those $34$ families and not the $3{,}478$.
