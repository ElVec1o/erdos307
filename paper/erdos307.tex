...
The second closure identifies which place of $\mathbb{Q}$ the problem lives at, by exhibiting a
setting in which the analogue is not merely unproved but \emph{false}.

\begin{theorem}[The ultrametric obstruction]\label{thm:ffnocycle}
Let $K$ be any field and define the arithmetic derivative on squarefree monic $f=\prod_i\pi_i\in K[t]$
by $f'=\sum_i f/\pi_i$. Then $\deg f'\le\deg f-1$. Consequently the arithmetic derivative on $K[t]$
has \emph{no cycle of any length} among non-constant squarefree polynomials, and the analogue of
\textup{\#307} is false over every $K[t]$.
\end{theorem}
\begin{proof}
Each summand $f/\pi_i$ has degree $\deg f-\deg\pi_i\le\deg f-1$, and the degree of a sum is at most
the maximum of the degrees. A cycle $f_0\mapsto f_1\mapsto\cdots\mapsto f_r=f_0$ of length $r\ge1$
would give $\deg f_0\le\deg f_0-r$, which is absurd.
\end{proof}
Verified by brute force independently of the proof: all $8192$ squarefree monics over
$\mathbb{F}_2$ of degree $\le13$ and all $6561$ over $\mathbb{F}_3$ of degree $\le8$ satisfy
$\deg f'\le\deg f-1$, with $0$ violations and $0$ two-cycles
\textup{(\texttt{code/ff\_nocycle.rs})}.

\begin{remark}[Why the existence route cannot be algebraic]\label{rem:archimedean}
The mechanism of Theorem~\ref{thm:ffnocycle} is the ultrametric inequality. The cycle equation is
...
that Theorem~\ref{thm:ffnocycle} refutes; hence no such argument exists. Together with
...
Theorem~\ref{thm:ffnocycle} kills the algebraic route by ultrametricity, which suggests replacing
...
one parameter and cannot be relaxed into a denser family. Theorem~\ref{thm:ffnocycle} removes the
