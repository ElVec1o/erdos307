...
so that $q=2(N\pm k)/m^2$ with $k^2=AB+Dm^2$ and $m\mid 4D$. Moreover $m$ is \emph{even}, and
consequently
\[ q\ \le\ N. \]
The extremal case is $m=2$, where $q$ is the larger root of $q^2-Nq+(D^2-D)=0$; writing
$\sigma=\sigma(S)=N/D$ and $t=\tfrac12\bigl(\sigma+\sqrt{\sigma^2-4}\bigr)$, that root is
$tD+O(1)$, so the largest admissible tail is the base times the very ratio $b/a$ that the mass
forces. At level $59$ this is $9.207\times10^{112}=1.0497\,D$, a factor $7.63$ below $4N$.
\end{proposition}
...
$q\ge N+1$ then $q^2-Nq-D\ge q-D>0$ because $D<N<q$, a contradiction. Hence $q\le N$. (Using only
$m\ge1$ in the last step gives $q\le4N$; the parity is what removes the factor four.)
\end{proof}
