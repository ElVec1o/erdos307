...
has $|P\cup Q|\ge 61$ and $\min(\prod P,\prod Q)>5.89\times10^{58}$: level $60$ is empty, closing both its
sectors, the pair sector by a mass bound on the free tail with no factorisation, and the single--tail sector by an
exact identity decided over $7{,}713$ admissible bases with no factorisation either. We add a parity dichotomy (a hypothetical all--odd
...
where a \#307 solution lies by rigidity; the unrestricted statement is not proved. This comes with a rate,
$\ll N(\log\log\log N)^{2+\varepsilon}(\log\log N)^{-1}$ for every $\varepsilon>0$, which rests on a
cancellation estimate uniform for moduli up to $(\log\log N)^2/(\log\log\log N)^{2+\varepsilon}$; the far stronger
$\ll N(\log\log N)^{1/2+\delta}(\log N)^{-1/4}$ also holds, by estimating the pretentious distance
without expanding in characters, which is uniform to power--of--log moduli. The
...
\emph{What this gives at the first open level.} At level $61$ the ladder reads $u_1\le30.5$, $u_{10}\le103.7$, $u_{30}\le201.1$, $u_{58}\le808.0$,
...
Corollary~\ref{cor:count59}, whose products satisfy $\log_{10}D\le113.397$
...
\emph{What the height means.} Proposition~\ref{prop:levelfinite} makes every level finite by an argument using only the two
...
\emph{What the window does not do.} It is worth recording what fails, since the natural guess is wrong. One might hope the mass window
...
\emph{The level--by--level programme survives its own barrier.} Together with Proposition~\ref{prop:levelbarrier} this splits the situation cleanly: at every level
...
\end{array}
\]
The sweep of Proposition~\ref{prop:nearmiss}, now carried to $10^{8}$, therefore already excludes
...
The barrier of Section~\ref{sec:barrier} bounds a solution; it does not exclude one. This section
...
\end{proposition}
\begin{proof}
Write $\sigma=N_S/D_S$, so $A_S=(\sigma+2)D_S$. Proposition~\ref{prop:tailbound} bounds
...
Theorem~\ref{thm:structure} removing the factor $\partial(\prod T)$, and is therefore a consequence
...
$\sum_{p\in S\setminus T}p^{-1}\equiv0\pmod r$, one for each $r\in T$. Each odd $r$ costs $1/r$, while the
congruence at $2$ holds exactly when $|T|\equiv|S|\pmod 2$, so the expected number of surviving $T$ of size $j$ is
$m_j=e_j+[\,j\equiv|S|\ (2)\,]\,e_{j-1}$, where $e_j$ is the $j$-th elementary symmetric function of the $1/p$,
$p\in S$ odd. Neither statement involves $A_S$, so the
...
so $2\in T$ forces $|T|\equiv|S|\pmod2$. $0$ \texttt{sorry}
...
$\gcd(\alpha,\partial(\alpha))=1$ by Theorem~\ref{thm:structure}.
\end{proof}

\begin{corollary}[The single--tail criterion is exact]\label{cor:splitexact}
Let $M=\{q\}$ with $q\ge3$ a prime not in $S$, and $\alpha^{2}<D$. Then $a=q\alpha$, $b=\beta$ is a two--cycle if and only if
$\alpha^{2}+\partial(\alpha)\partial(\beta)=D$ and $q=\partial(\beta)/\alpha$. If $|S|$ is odd, $2\in S$ and
$\Sigma=\sum_{p\in S}1/p<2.01$, the identity holds if and only if $D\mid\alpha^{2}+\partial(\alpha)\partial(\beta)$, that is, if
and only if $\sum_{p\in S\setminus T}p^{-1}\equiv0\pmod r$ for every $r\in T$ and
$\alpha^{2}+\partial(\alpha)\,\beta/r\equiv0\pmod r$ for every $r\in S\setminus T$; and every two--cycle has $|T|$ odd.
$0$ \texttt{sorry} for the algebraic core \textup{(\texttt{Erdos307.split\_cycle\_of\_identity},
\texttt{split\_card\_odd}; \texttt{lean/Erdos307/SplitExact.lean})}.
\end{corollary}
\begin{proof}
The forward direction is Proposition~\ref{prop:splitsieve}. Conversely, the identity reduced modulo $\alpha$ gives
$\alpha\mid\partial(\beta)$ as in its proof; with $q=\partial(\beta)/\alpha$ this is $\partial(b)=a$, and substituting
$\partial(\beta)=q\alpha$ into $\alpha\beta-\alpha^{2}=\partial(\alpha)\partial(\beta)$ and cancelling $\alpha$ gives
$\beta=\alpha+q\,\partial(\alpha)=\partial(q\alpha)$, which is $\partial(a)=b$. For the divisibility, put
$N=\alpha^{2}+\partial(\alpha)\partial(\beta)$; then $N/D=\alpha^{2}/D+\sigma(T)\sigma(S\setminus T)<1+\Sigma^{2}/4<2.02$, and
$N=2D$ would give $\alpha^2/D\ge2-\Sigma^2/4$, hence $q=\sigma(S\setminus T)D/\alpha^{2}\le\Sigma/(2-\Sigma^{2}/4)<3$. The local
form reduces $N$ modulo each prime of $S$, using $\partial(\beta)\equiv\beta/r\pmod r$ for $r\in S\setminus T$. For the parity:
if $2\in T$ this is Proposition~\ref{prop:splitsieve}; if $2\notin T$, then $\alpha$ and $q$ are odd and
$\partial(\alpha)\equiv|T|\pmod2$, so $\beta=\alpha+q\,\partial(\alpha)\equiv1+|T|$ is even only for $|T|$ odd.
\end{proof}

The $58$ character conditions of the tail at the odd primes of $S$ add nothing to the square of
Proposition~\ref{prop:lvl60factor}: $\partial(D)\equiv D/p\pmod p$ turns $x^{2}=A_Sq+D_S$ into $x^{2}\equiv q\,D_S/p$, a nonzero
square modulo $p$, which is the condition $(q\mid p)=((D_S/p)\mid p)$.

\emph{What the sieve decides.} The criterion was applied to every level--$60$ single--tail base surviving the certificate of
...
observed survivor count is $5.20$ per base against the $5.26$ of the model $m_j$, which matches size by size
\textup{(\texttt{code/split\_sieve.rs}, \texttt{code/gen\_bases.rs})}. On the $7{,}713$ bases that also
survive trial division to $10^{6}$, the pass was extended to $|T|\le8$: $2.0\times10^{13}$ splits,
with $37$ survivors at $|T|=7$ against $39.1$ predicted, none at $|T|=8$ against $0.51$, and again no two--cycle. The range
$9\le|T|\le34$, where the model expects $0.57$ survivors over all $7{,}713$ bases, is not decided by enumeration, since
$\alpha^{2}<D$ bounds $|T|$ by $34$, so the sieve reduces the level--$60$ single--tail question without closing it.
...
$\sigma=N_S/D_S$ and $A_S=(\sigma+2)D_S$. Proposition~\ref{prop:tailbound} gives $q\le tD_S$ with
...
pair--sector bases of Proposition~\ref{prop:sectors}(ii), Corollary~\ref{cor:maxbound} bounds the tail
...
$3.2\times10^7$ core--years the divisor search of Proposition~\ref{prop:tailbound} would demand, a
...
Proposition~\ref{prop:sectors}; the other, the pair sector, is empty by Proposition~\ref{prop:pairclosed}, without
factoring. What has changed is the character of the obstruction: level $60$ is no
...
Proposition~\ref{prop:close59} being the $59$--prime sets of mass exceeding $2$; the complement is
...
an arity--\emph{one} family in $m$; Proposition~\ref{prop:pairlocal} concerns only the family with both new primes free.
...
Every base is decided by Proposition~\ref{prop:pairclosed}, which tests its tails one by one and needs no certificate
(\texttt{code/pairsector\_kill.rs} in campaign mode reproduces the $49{,}961$, $31{,}219$ and $18{,}742$ above).
\end{remark}

\begin{proposition}[Level $60$ is empty]\label{prop:level60closed}
No solution of \#307 has $|P\cup Q|=60$. Consequently every solution has $|P\cup Q|\ge61$.
\end{proposition}
\begin{proof}
By Proposition~\ref{prop:sectors} a $60$--element support of mass above $2$ lies in the single--tail sector or the pair
sector, and the latter is empty by Proposition~\ref{prop:pairclosed}. In the single--tail sector write
$a=q\alpha$, $b=\beta$, $\alpha\beta=D=\prod S$, $\rho=\alpha^{2}/D<1$. For $\rho\le2^{-40}$ the identity of
Proposition~\ref{prop:splitsieve} gives $\sigma(T)\bigl(\Sigma-\sigma(T)\bigr)=1-\rho$, which confines $\sigma(T)$ to a window of
width about $\rho/\sqrt{\Sigma^{2}-4}$ around a root of $X(\Sigma-X)=1$; every $T$ in the window, at every $|T|$, is enumerated by
a meet--in--the--middle over four quarters of $S$ and decided exactly. For $\rho>2^{-40}$ the tail is bounded,
$q=\sigma(S\setminus T)/\rho<\Sigma\,2^{40}$, and the square of Proposition~\ref{prop:lvl60factor} fixes $(q\mid p)=((D/p)\mid p)$
at the $58$ odd $p\in S$; every $q$ in range is sieved by these conditions and each survivor decided by the two squares.
Over the $7{,}713$ bases, a superset of the $3{,}444$ open families, the first search examines $4.50\times10^{11}$
candidates and finds no $T$ satisfying the identity, and the second examines $2.27\times10^{13}$ candidates and leaves no
$q$ making both squares perfect. The last claim is Proposition~\ref{prop:sectors} with the barrier of
Theorem~\ref{thm:barrier}.
\end{proof}
The computations are \texttt{code/splitmitm/split\_mitm.rs} and \texttt{code/qsieve/q\_sieve.rs}, the latter reproduced base
by base by the independent \texttt{code/qsieve/q\_sieve2.rs}, with identical survivor lists (all empty) on every base.
Both are audited: \texttt{code/splitmitm/stageA\_exact.gp} re-decides every reported split by the identity directly and
confirms exactly one completed pass per base with no candidate misreported; \texttt{code/qsieve/q\_exact.gp} does the same
for the tail sieve; and \texttt{split\_mitm --audit} plants $100$ genuine two--cycle--shaped splits across $5$ open bases and
recovers all $100$. The assembly from level $60$ to the bound $61$ is formalised
(\texttt{Erdos307.erdos307\_sixtyone\_of\_level60}).

\begin{corollary}[Sharpened bound]\label{cor:level61bound}
Every solution has $\prod_{p\in U}p\ge\Pi_{61}>6.98\times10^{117}$ and
$\min(\prod P,\prod Q)\ge\sqrt{\Pi_{61}/T_{61}}=5.8918\ldots\times10^{58}$.
\end{corollary}
\begin{proof}
Repeats Proposition~\ref{prop:close59}'s bound on the minimal side with the minimum of $R/T$ now over $|U|\ge61$,
attained at the first $61$ primes.
\end{proof}
...
\begin{proposition}[The pair sector of level $60$ is empty]\label{prop:pairclosed}
No $60$--element support in the pair sector of Proposition~\ref{prop:sectors} carries a two--cycle.
\end{proposition}
\begin{proof}
Write the support as $U=R\cup\{m\}$ with $m=\max U$ and $D=\prod R$, and let $N=ab=Dm$ with $a'=b$, $b'=a$, where $a,b$
are coprime and squarefree (Theorem~\ref{thm:structure}). By Proposition~\ref{prop:sectors}\textup{(ii)},
$T(R)<2<T(R)+1/m$, that is
\[ \max R<m<\frac{1}{2-T(R)}. \]
Now $N'=a'b+ab'=a^2+b^2$. Let $p\in R$ be odd. It divides exactly one of $a,b$, so $N'$ is a nonzero square modulo $p$.
Since $p\mid D$ and $p\nmid m$, $N'=D'm+D\equiv(D/p)\,m\pmod p$, hence
\[ \Bigl(\frac{m}{p}\Bigr)=\Bigl(\frac{D/p}{p}\Bigr). \]
Every base contains $2$, since $59$ odd primes have reciprocal sum below $1.51$, so these are $58$ conditions. For none
of the $18{,}234{,}653$ bases $R$ of Remark~\ref{rem:campaign} does an odd $m$ in the displayed range satisfy all $58$:
the ranges are short, $1/(2-T(R))<1.14\times10^{8}$, and the $2{,}143{,}165{,}628$ pairs $(R,m)$ are checked by table
lookup with no survivor.
\end{proof}

The check is \texttt{code/pairsector\_close.rs} (one thread, about four CPU--minutes). It decides sector membership and
the range of $m$ in exact integer arithmetic, with $0$ ambiguous cases, recovers a planted tail and the count
$18{,}234{,}653$, and agrees with an independent implementation on every count. The algebraic core is formalised
(\texttt{Erdos307.pair\_squares}, \texttt{pair\_tail\_bound}, \texttt{pair\_char\_square}, \texttt{pair\_legendre};
\texttt{lean/Erdos307/PairClosed.lean}). Proposition~\ref{prop:pairlocal} is not contradicted: it concerns the family
with both new primes free, and here the sector's own mass condition bounds the one tail that remains. Level $60$ is
thereby reduced to the $3{,}444$ open single--tail families.
...
\begin{proposition}[Why a one--sided mass argument cannot reach the barrier]\label{prop:twosided}
...
(Corollary~\ref{cor:threethirteen}, the case $u=1$). Hence no argument that uses only $a'=b$ with $b$
...
consume the second equation $b'=a$. This is why the mass arguments of Section~\ref{sec:barrier}, and the
...
space: Proposition~\ref{prop:tailbound} bounds $q$ itself, so the true sum is finite, and
...
Proposition~\ref{prop:tailbound} had already closed; the kills remove ${\sim}93\%$ of the count's mass,
and Proposition~\ref{prop:immunedecide} removes the rest of the $34$ conditionally immune families'
contribution unconditionally, both candidate values of $q$ failing outright, and the pair sector is empty by
Proposition~\ref{prop:pairclosed}. The level--$60$ part of the count's ``weakly favours \textsc{yes}'' signal is therefore
zero; the count itself, which ignores the $N(r)\le1$ rigidity, is unaffected.
\end{remark}
...
Remark~\ref{rem:whatisleft} names the exponential Pell orbit of Remark~\ref{rem:lehmer} as the only live branch, and it is natural to read the surviving question as the primality of a term $q_n$ along it. That reading does not hold. The orbit is genuinely infinite and both square conditions do hold along all of it; what fails is that no term past $N$ can be prime, by Proposition~\ref{prop:tailbound}. The single identity responsible is $A-B=4D$, used elsewhere only to form the Pell
system and never to factor $4Dq$.
\emph{The threshold in largest--prime coordinates, and which sectors it leaves alive.} Remark~\ref{rem:tier60} already writes a cycle as $a=qm$ with $q$ the largest prime; Bado
...
Proposition~\ref{prop:close59} supplies once $d$ is small: at $q=281$ the first row would read
$1295$.\textup{)}
...
Corollary~\ref{cor:threethirteen} they are the $u=1$ solutions of \#307's coprime relaxation, so
...
Proposition~\ref{prop:tailbound}, with $\alpha=d$ and $\beta=e$. Substituting $e=d+qd'$ and $e'=qd$
...
\textup{(\texttt{code/sector\_dprime.gp})}. This is Theorem~\ref{thm:barrier}'s constant recovered
...
By the sector identity, $\sigma(e)=T-1764/(41e)$, and $e>3.5\times10^{57}$ by Theorem~\ref{thm:barrier},
...
not move Theorem~\ref{thm:barrier}, whose $60$ is proved by a different route and over all supports;
...
Proposition~\ref{prop:tailbound} bounds the tail prime by $N$.
...
question, of the same class as the $p=5$ family of Proposition~\ref{prop:plus}\textup{(iii)}, and
...
$p^2$, and that needs the fundamental solution, the object Remark~\ref{rem:untestable}\,(i) places
...
\emph{A cautionary refinement.} One can try to sharpen this into a quantitative rate using a Kubilius/Erd\H{o}s--Wintner model,
...
\emph{Two dynamical probes: no second obstruction.} Two measurements sharpen the picture beyond the first--order mass count, and both cut towards rather
...
\emph{Why the analytic method is starved: $N(r)\le1$.} The rank--one rigidity (Section~\ref{sec:status}) has a sharp consequence for the circle--method
...
Proposition~\ref{prop:splitdisc} (which over $\mathbb{Z}$ carries the entire obstruction, via
...
\texttt{Erdos307.Extremal} proves it
...
forced prime. What that leaves unexplained is the shape of the examples that \emph{are} known, $1=(1+\frac15)(\frac12+\frac13)$, and the family generated by primary pseudoperfect numbers such as
...
power $p^e$ with $p\nmid\alpha$, Lemma~\ref{lem:symbolfact} gives
...
Lemma~\ref{lem:symbolfact} with $r=p^e$, legitimate because $p\nmid\alpha$.
\end{proof}
...
The same tension defeats the complementary route, which uses Lemma~\ref{lem:symbolfact} in the
...
\begin{proposition}[The ten--factor primary pseudoperfect number does not extend by one or two primes]\label{prop:ppninherit}
Let $N_{10}=N_9(N_9+1)=35979351189199316534587473905773572006$ be the primary pseudoperfect number with
ten prime factors of \cite{PortFillings2026}. There is no prime $p\nmid N_{10}$ with $N_{10}p$ primary
pseudoperfect, and no pair of distinct primes $p,q\nmid N_{10}$ with $N_{10}pq$ primary pseudoperfect.
$0$ \texttt{sorry} \textup{(\texttt{Erdos307.N10\_no\_one\_prime}, \texttt{N10\_no\_two\_prime};
\texttt{lean/Erdos307/PPNInherit.lean})}.
\end{proposition}
\begin{proof}
The idea is that inheritance is a factorisation problem rather than a search: the new primes are
pinned to a divisor pair of one fixed integer, so exhausting that integer's divisors decides the
question. Write $K=N_{10}$, so $K'=K-1$. By Leibniz, $(Kp)'=K+p(K-1)$, and $(Kp)'=Kp-1$ forces
$p=K+1$, which is divisible by $7$. For two primes, $(Kpq)'=Kpq-1$ is equivalent to
$(p-K)(q-K)=K^{2}+1$. Both factors have the same sign; if both are negative then
$(K-p)(K-q)\le(K-2)^{2}<K^{2}+1$, so both are positive and $d=\min(p,q)-K$ is a divisor of
\[ K^{2}+1=21807157\cdot480382349\cdot P_{60},\qquad
   P_{60}=123572138719194583969192220095883252267503088389616114960309, \]
all three factors prime. Hence $d\in\{1,\,21807157,\,480382349,\,21807157\cdot480382349\}$, and
$K+d$ is divisible by $7,7,5,2141$ respectively, so it is not prime. The primality of $P_{60}$ and of
$N_9+1$ is certified by Pratt trees whose leaves lie below $10^{9}$
\textup{(\texttt{lean/Erdos307/PrattN10.lean}, \texttt{code/pratt\_n10.gp})}.
\end{proof}

The proposition is the frontier case of a routine that is validated on known values before it is
used there: the same criterion, run on every primary pseudoperfect number with at most ten prime
factors, recovers $42$ from $2$, $1806$ from $6$, and the eight--factor
$2214502422\cdot2217342227\cdot1729101023519$ from $2214502422$, and returns nothing at $N_9$ or $N_{10}$
\textup{(\texttt{code/ppn\_inherit.py})}. It bears on the conditional infinitude theorem of
\cite{PortFillings2026}, whose hypothesis splits a terminal prime into five: at the current frontier
the one-- and two--prime steps are exhausted rather than unsearched. Three primes would require
factoring $K^{2}x^{2}+x-K$, about $150$ digits, for each candidate prime $x>K$.

\emph{Why \#313 has a family and the $1$--free case has none.} Proposition~\ref{prop:portfixed} explains the asymmetry between
...
\emph{What the relaxation localises.} The relaxation is worth recording because it is natural to expect the mass condition to be an
...
\emph{Why level $61$ is still not enumerable.} This is the complement of Proposition~\ref{prop:window}, which places the mass of a \emph{solution}
...
\emph{What actually obstructs, in both problems.} It is worth being exact about where the difficulty sits, because the port dynamics is not where it
...
\emph{The price of dropping the member $1$.} The reduction is exact and factoring--free, but it fixes the size. Since
...
\emph{Where the neighbouring literature stands, and what it supplies.} Three recent items bear on the problem without deciding it, and it is worth saying exactly what each
...
can be approached, which is the direction Proposition~\ref{prop:effapprox} occupies, and says
...
and the barrier are Lean--verified, extremality proved rather than assumed. Level $60$ is then shown empty
(Proposition~\ref{prop:level60closed}), both its sectors closing without factorisation, which sharpens the barrier to
$|P\cup Q|\ge61$ and $\min(\prod P,\prod Q)>5.89\times10^{58}$ (Corollary~\ref{cor:level61bound}).
...
$|U|\le68$ (Proposition~\ref{prop:split}); and in the level, the forced core and the ceiling that
...
(Theorem~\ref{thm:a9}, Proposition~\ref{prop:condrate}), and likewise the plus layer, so the
squarefree Pythagorean layer has density zero unconditionally. The route is not the square sieve of
Appendix~\ref{app:sieve}, whose analytic input is foreclosed at its range by
Remark~\ref{rem:charcancelerratum}; it is a sieve at the far smaller
$P=\log\log N/(\log\log\log N)^{1+\varepsilon}$, where two uniform estimates \emph{are} available, Lemma~\ref{lem:charcancelunif} for the character sums and Lemma~\ref{lem:deficit} for the local
deficits. The obstruction that remark records is range--dependent rather than
...
Theorem~\ref{cor:a9rate} sharpens it to
$N(\log\log N)^{1/2+\delta}(\log N)^{-1/4}$, a full quarter--power of $\log N$ better. What made that
possible is Lemma~\ref{lem:swdirect}: the pretentious distance can be estimated by applying
...
revisions are retracted after Lemma~\ref{lem:reversal} and at the theorem itself. Nothing else in this paper
...
Open problems: (i) close level $61$ as level $60$ was closed (Proposition~\ref{prop:level60closed}: the
pair sector by a mass bound with no factorisation, the single--tail sector by an exact identity decided over every
admissible base with no factorisation either; the finite verification of
...
\emph{The place of the problem: a local--global dictionary.} The pieces above assemble into a dictionary. \textup{(i)} For a fixed support $U$, the subset form of
...
\emph{Where a proof of \textsc{no} would have to come from.} Four results of this note combine into a single statement about the shape any negative proof must
...
any finite combination of moduli. Theorem~\ref{thm:ff} shows the analogue over $\mathbb{F}_q[t]$ is
...
Proposition~\ref{prop:noplace} shows no place carries a descent. Together these localise the
...
\begin{remark}[What the single equation leaves to work with]\label{rem:closedroutes}
A two--cycle carries exactly one relation. If $\sigma(P)\sigma(Q)=1$ then $a'b'=ab$, and
$\gcd(a,a')=\gcd(b,b')=1$ gives $a\mid b'$ and $b'\mid a$, so $b'=a$ and $a'=b$; the converse is
immediate. Four consequences follow.
\begin{enumerate}
\item No congruence between $a$ and $b$ beyond the equation is available from the primes of the support:
for $r\mid a$, reduction modulo $r$ and modulo $r^{2}$ returns $b\equiv a'$, which holds for every $a$.
A counting bound in the style of Pomerance's for Lehmer's totient problem therefore has no divisibility to
start from, and the fibres of $n\mapsto n'$ contain the Goldbach sets $\{pq:p+q=v\}$, so no uniform fibre
bound replaces it.
\item No second identity exists: $\sum1/p^{2}$ enters no consequence of the equation, and any
second--moment statement is an inequality about which supports have $\sigma$ near a given value.
\item Near misses fail on the far side. For the $289$ even squarefree $A$ reached from $2$ by steps
keeping the defect $A-A'\le200$ and stopped past $10^{40}$, the candidate partner $b=A'$ is never squarefree with $b'=A$: $34$
are not squarefree and in the other $255$, $b'<A$, by more than $0.63A$. The primary pseudoperfect numbers
$6,42,47058,2214502422$ give $\sigma(b)=0.2,\ 0.024,\ 2.1\times10^{-5},\ 0.053$
\textup{(\texttt{code/nearmiss\_cycle.gp})}.
\item The populated model shows no hidden divisibility. Over the $10{,}292$ distinct coprime norm pairs
$(A,B)$ of the twisted two--cycles of $\mathbb{Z}[\sqrt2]$ (Proposition~\ref{prop:sunit}), and conditional on
$\ell\nmid AB$, the frequency of $\ell\mid A\pm B$ equals that of random norms of the same size for
$\ell=3,5,7,17,23,31$; every raw bias is explained by which primes the support contains
\textup{(\texttt{code/sqrt2\_auxmod.gp})}.
\end{enumerate}
The only arithmetic a proof can use is therefore the prime content of the support, and its first moment
is exhausted at $|U|\ge60$.
\end{remark}
...
Proposition~\ref{prop:ladder}); the intrinsic union criterion (his Thm.~7.3, our
...
\texttt{split\_product} the two--layer form of Proposition~\ref{prop:splitdisc}, and
...
Pythagorean equation itself rather than an assumed mass bound.
...
filter of Corollary~\ref{cor:qr} is \texttt{qr\_filter}, resting on \texttt{cofactor\_survival},
the congruence $N'+2N\equiv N/\ell\pmod\ell$, which is proved rather than assumed (stating \texttt{qr\_filter} with that arithmetic as a hypothesis on abstract residues would leave it contentless); Lemma~\ref{lem:symbolfact} is \texttt{Frame.csum\_eq\_mul\_recipSum} for its first half
...
families of Remark~\textup{\ref{rem:campaign}} are precisely the tail families the congruence
...
Remark~\textup{\ref{rem:lehmer}}, deciding whether $B_Sx^2-A_Sy^2=-4D_S^2$ has an integral solution
...
Proposition~\ref{prop:tailbound} forces $q\le tD_S$ and hence $x<A_S$
(Proposition~\ref{prop:lvl60factor}), so $t=0$ and the two square roots $x_0$ and $A_S-x_0$ exhaust
the parametrisation. The decision is therefore a pair of primality tests, not an unbounded sweep,
and Proposition~\ref{prop:immunedecide} carries it out: all $34$ immune families are empty. The
...
local solubility of Remark~\ref{rem:tier60} rational solubility is unconditional; the reply was that
what is missing is integrality, not solubility. Both sides of that exchange were beside the point.
Integral points are not needed either, because Proposition~\ref{prop:tailbound} confines the solution
to a box containing at most two candidates, and Proposition~\ref{prop:immunedecide} simply tests
...
Proposition~\ref{cor:kdetermined} confines $k$ to $\{0,-1\}$ is not a restriction anywhere a search
...
The mass window transfers to the normal form as well. In Proposition~\ref{prop:oneprime} a solution
...
\emph{The conditional shape of the near--miss law.} Proposition~\ref{prop:nearmiss} records the
maximum of $r(a)=\sigma(a)\sigma(a')$ by decade. The conditional maximum, given the value of
$\sigma(a)$, is more informative, because it is where the two--sided obstruction becomes visible.
Over the squarefree $a\le8\times10^{6}$ with $a'$ squarefree and $\sigma(a)\in[0.90,1.12]$, the
largest $\sigma(a')$ in each bucket of width $0.01$ decreases through the critical region, from
$0.5481$ at $\sigma(a)=0.90$ to $0.3110$ at $\sigma(a)=1.12$, and equals $0.4436$ at
$\sigma(a)=1.00$, where a solution requires $1$. Quadrupling the range from $2\times10^{6}$ leaves
the bucket maxima at $\sigma(a)=1.00$ and $1.05$ unchanged, at $0.4436$ and $0.3786$, and moves the
window maximum of $r$ only from $0.5016$ to $0.4955$, attained at $a=5{,}767{,}190$
\textup{(\texttt{code/twoside\_conditional.rs})}.

The mechanism is the barrier seen from the other side. A mass $\sigma(a)$ near $1$ requires $a$ to
carry the small primes; $\gcd(a,a')=1$ then denies them to $a'$, whose primes are larger and whose
mass is correspondingly smaller. This is why the primary pseudoperfect numbers, which satisfy the
one--sided equation with $|\sigma(n)-1|=1/n$ and so refute any one--sided route to an absolute
constant, never come near a cycle: for them $a'=a-1$ is prime or nearly prime and $\sigma(a')$
collapses, at $a=47{,}058$ to about $2\times10^{-5}$. The suppression is not special to that family
but holds across the window. The label is \textup{HEURISTIC}: by Proposition~\ref{prop:nearmiss} the
region $r\in(0.998740,\infty)$ lies above the barrier, so no sweep tests the claim where it would
matter, and the measurement bounds nothing.
...
families could not be examined, and Proposition~\ref{prop:immunedecide} examines them and finds them
...
input does not close in the form attempted there, and why Remark~\ref{rem:whatisleft} keeps this
...
Theorem~\ref{thm:barrier} is under one order, is an \emph{upper} bound on a minimum, and an upper
...
$\sigma(m)=2$ exactly, impossible for $m>1$ since $\gcd(D(m),m)=1$ (Theorem~\ref{thm:structure})
...
\textup{(Label: \textsc{proved}. The input that makes it unconditional is Lemma~\ref{lem:swdirect}.)}
For every $\delta>0$,
