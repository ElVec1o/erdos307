...
and $h_t$ is multiplicative on squarefree $m$ with $|h_t|=1$, which is what puts it inside
Hal\'asz's theorem.
...
infinity, and here the key point is that the estimate is \emph{uniform in $\tau$}. Note first that $h_t$ is \emph{not} a function of $m\bmod r$, since it is built from the
...
$\tau$; the non--principal terms are $O_r(\log\log\log N)$ for $|\tau|\le\log N$. Their smallness comes
...
at $r=3$ \textup{(\texttt{code/charcancel\_check.gp})}. So $M\gg\log\log N$ uniformly in $\tau$, and
