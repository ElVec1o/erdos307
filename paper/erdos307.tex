...
The argument for Theorem~\ref{thm:a9} is a square sieve whose analytic input does not close, which
is why the theorem stands at \textsc{conjecture} and why Remark~\ref{rem:whatisleft} keeps it off
the critical path. It occupied eight pages in the main line, ahead of results that are proved. It is
...
\section{The square sieve: the argument for the minus-layer density theorem, and where it fails}\label{app:sieve}

Relegated from \S\ref{ssec:sieve}. Theorem~\ref{thm:a9} is labelled \textsc{conjecture}: the
sieve below is a complete argument except for its analytic input, and that input is not supplied
here. It is kept in full because the algebraic layer is correct and reusable, and because the
location of the gap is itself the useful information.
...
\textup{(Label discipline, as for Theorem~\ref{thm:a9}: this proof has been through two rounds of
adversarial review; each found defects that were repaired, so under the project's standard the
statement does not yet carry \textsc{proved}, which requires a round that finds nothing.)}
Fix $\delta\in(0,1)$. For every odd squarefree $r$ with $7\le r\le(\log\log N)^{2-\delta}$,
\[ T_r(N)\;\ll\;N\,(\log N)^{-2/5}, \]
with an implied constant that is absolute but ineffective; the same bound, by the same proof with
$e_r(+2t)$ in place of $e_r(-2t)$, holds for the sum with $(m'+2m\mid r)$ in place of
$(m'-2m\mid r)$. For $r\in\{3,5\}$ the same holds with
exponents $c_3-\varepsilon=0.133\ldots$ and $c_5-\varepsilon=0.4409\ldots$ in place of $2/5$, as in
Remark~\ref{rem:charcancelsharp}.
\end{lemma}
\begin{proof}
The proof of Lemma~\ref{lem:charcancel} is followed step by step; what is new is that every
implied constant is tracked in $r$, and each is absolute in the stated range. Write
$L=\log\log N$, so $r\le L^{2-\delta}$ and $\log r\le(2-\delta)\log L$.
...
\[ |s-\beta|\;\ge\;\operatorname{Re}(s)-\beta\;>\;1-\beta\;\ge\;c(\varepsilon)\,r^{-\varepsilon} \]
by Siegel's theorem in zero--location form, so
$|\log(s-\beta)|\le\log\bigl(1/(1-\beta)\bigr)+O(1)\le\varepsilon\log r+O_\varepsilon(1)
\ll\varepsilon\log L+O_\varepsilon(1)$; the decomposition is \emph{not} asserted beyond $|\tau|\le1$,
...
$\sum_{\psi\ne\psi_0}|c_\psi|\le\sqrt{\varphi(r)}\le L^{1-\delta/2}$, so the non--principal
contribution to the distance is $\ll L^{1-\delta/2}\log L=o(L)$, and
\[ M\;\ge\;\Bigl(1-\frac{\sqrt r}{\varphi(r)}\Bigr)L\;-\;O\bigl(L^{1-\delta/2}\log L\bigr)
...
$r\ge7$: the minimum over that range is $1-\sqrt{15}/8=0.515\ldots$ at $r=15$, since each local
factor satisfies $(p-1)/\sqrt p\ge2/\sqrt3$ and only $r=3,5$ fall below; swept directly over all odd squarefree $r\le20001$
\textup{(\texttt{code/uniform\_min\_sweep.gp})}. Hal\'asz
...
every computable $N$: the crux inequality $L^{1-\delta/2}\log L\ll L$ first bites, at
$\delta=0.2$, around $\log\log N\approx e^{36}$. And the constant is ineffective twice over,
through Siegel--Walfisz and through Siegel's theorem; no effective form is claimed.
\end{proof}

\begin{conjecture}[Two uniform deficit lemmas]\label{conj:deficit}
Let $\delta\in(0,1)$ and $c\in\{2,-2\}$. Uniformly for primes $p\ne q$ in
$(\tfrac12(\log\log N)^{1-\delta},(\log\log N)^{1-\delta}]$, with absolute implied constants,
...
The residue enters the sketched route only through the phase $e_p(-ct)$, so the two values of $c$
are on the same footing.
The natural route is the additive--character analogue of Lemma~\ref{lem:charcancelunif}: the
detector $e_p(t\sigma_p(m))$ makes the summand multiplicative in $m$, the principal Fourier
coefficient of $a\mapsto e_p(ta^{-1})$ is a Ramanujan sum, $-1/(p-1)$, so the distance constant is
$1-1/(p-1)$, larger than in Lemma~\ref{lem:charcancelunif}; Lemma~\ref{lem:localsq} runs the same
computation at fixed modulus. The uniform write--up has not been carried out, and after three
failed reviews of Theorem~\ref{thm:a9}'s text no step of it will be assumed routine.
\end{conjecture}

\begin{proposition}[A rate on the minus layer, conditional on the deficit lemmas]\label{prop:condrate}
Assume Conjecture~\ref{conj:deficit}. Then, for every $\delta\in(0,1)$,
\[ \#\{m\le N:\ \mu^2(m)=1,\ m'-2m\ \text{a positive perfect square}\}
   \;\ll\;N\,\frac{\log\log\log N}{(\log\log N)^{1-\delta}} , \]
and the same for $m'+2m$. In particular Theorem~\ref{thm:a9} would follow with a rate, by an
argument with no diagonalisation over $r$.
\end{proposition}
\begin{proof}
Let $P=(\log\log N)^{1-\delta}$, $\mathcal P$ the primes in $(P/2,P]$, $\Pi=|\mathcal P|\gg
...
Lemma~\ref{lem:charcancelunif} at $r=pq\le(\log\log N)^{2-2\delta}$ (each $r$ odd squarefree,
$r\ge7$), $\sum_{p\ne q}|T_{pq}|\ll\Pi^2N(\log N)^{-2/5}$. By Conjecture~\ref{conj:deficit},
...
bound follows. Note $\mu^2(m)$ and the coprimality conditions are carried inside every count;
