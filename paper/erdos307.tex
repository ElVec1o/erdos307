...
Hal\'asz gives $\sum_mh_t(m)\ll N\bigl((1+M)e^{-M}+(\log N)^{-1/2}\bigr)=o(N)$. Summing the $r$
values of $t$ and dividing by $|\tau(\chi)|=\sqrt r$ gives $T_r(N)=o_r(N)$.
\end{proof}
...
The following rate does \emph{not} follow from Lemma~\ref{lem:charcancel} as stated there, because
the uniform-in-$\tau$ bound its proof consumes is false near the threshold; see the erratum after
that lemma. The statement is retained as a conjecture, and nothing in this paper depends on it.
