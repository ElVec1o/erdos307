...
\emph{not} a feasible finite search. On the counting side we prove that both Pythagorean layers have density zero
...
a convenience: every tool below is squarefree--only, and an earlier revision wrote these sums
unrestricted, which made the identity with $T_{pq}+E_{pq}$ false by the non--squarefree terms. Those
are not negligible, being empirically of size $N^{0.9}$ per pair against a $T_{pq}$ of size
$N^{0.6}$, and the paper bounds them by nothing better than the trivial $(1-6/\pi^2)N$.
...
statement ``$\ll N/p^2$ for each fixed $p$'' advertises exactly such a constant and was cited here
in an earlier revision. Summing, $\sum_{p\in\mathcal{P}}p^{-2}\le\Pi/P^2\asymp1/(P\log P)$. Here
...
established here. It is independently blocked twice over: by the regime mismatch recorded after
Lemma~\ref{lem:reversal}, and by Lemma~\ref{lem:localsq}, which at $P=P(N)$ would have to be uniform
in $p$ and is not, its $o(1)$ being driven by non--principal $L$--functions modulo $p^2$ and
ineffective in $p$. Theorem~\ref{thm:condpower} meets the same obstruction and sidesteps it with a
divisor bound instead. The statement is retained as a conjecture, nothing in this paper depends
...
\[ \#\{m\le N:\ \mu^2(m)=1,\ m'-2m\ \text{is a positive perfect square}\}
    \;\ll\;\frac{N\log\log N}{(\log N)^{1/4}}. \]
The squarefree restriction is inherited from Theorem~\ref{thm:a9} and from
Lemma~\ref{lem:charcancel}; an earlier revision stated this unrestricted.
\end{corollary}
...
sum, as Lemma~\ref{lem:charcancel} records, and an earlier revision listed them here in error. Note also that the theorem does not decide \#307, and cannot: it is a statement about
