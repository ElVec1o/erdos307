...
so that $q=2(N\pm k)/m^2$ with $k^2=AB+Dm^2$ and $m\mid 4D$. In particular
\[ q\ \le\ 4N. \]
\end{proposition}
...
Finally $m\ge1$ and $c^2\ge0$ give $4Nq=q^2m^2+c^2-4D\ge q^2-4D$, so if $q\ge4N+1$ then
$q^2\ge4Nq+q>4Nq+4D$ because $q>4N>4D$, a contradiction. Hence $q\le4N$.
\end{proof}
...
both square conditions do hold along all of it; what fails is that no term past $4N$ can be prime.
...
along a real orbit, in \texttt{code/tailbound.gp}: on the base $D=1$, $N=7$ (bound $4N=28$) the orbit
...
\emph{The count reverses sign.} The expected number of admissible $m$ is
$\sum_{m\mid 4D}(AB+Dm^2)^{-1/2}\approx D^{-1/2}\sum_{m\mid4D}m^{-1}$, which on the first immune base
is $2.4\times10^{-56}$ and below $10^{-38}$ summed over all $34$ immune families. The finite search is
therefore expected to be empty on every one of them, which is the opposite of the ``weakly favours
\textsc{yes}'' of Remark~\ref{rem:killcount} on exactly the families that reading was about.
...
a Pythagorean tail prime. Neither is a proof: the bound $q\le4N$ is proved, the emptiness is
\textsc{heuristic}.
...
\begin{remark}[The immune signal, and why its Lehmer reading fails]\label{rem:lehmer}
The reading of this remark is superseded by Proposition~\ref{prop:tailbound} and
Remark~\ref{rem:notlehmer}: everything below about the \emph{orbit} stands, but the inference that
primality along it is the residual question does not, because the tail prime is bounded by $4N$. It
is kept because the orbit, its growth rate and its three gates are what the bound is a statement
about. The immune families of Remark~\ref{rem:killcount} do not merely survive the sieve; they carry
a definite arithmetic shape. For a reciprocity--immune base the Pell system of Remark~\ref{rem:tier60},
...
Remark~\ref{rem:lehmer}. It is alive but narrower than it was.
...
\emph{measured} at a single modulus is smaller and flat. At $r=5$, $t=1$ the deficit between
...
\textup{Label: \textsc{conjecture}, and this is a considered downgrade rather than a known error.}
The argument below has survived four rounds of adversarial review at the level of its
\emph{mathematics}: the square sieve, the $E_{pq}$ correction, the diagonal bound, the limsup
argument and the whole of Lemma~\ref{lem:charcancel}'s algebra were each attacked and each held. What
has not held is the \emph{text}. In three consecutive rounds a load-bearing display was found to have
silently dropped a hypothesis, twice the coprimality condition and once the squarefree condition, and
each repair introduced a fresh defect elsewhere. A statement whose written proof has needed a
correction in every round of review, with no sign of the rate declining, has not earned
\textsc{proved}, whatever one's private confidence in it.

The honest position is therefore: the author believes the statement and believes the argument, and
does not claim either as established. Nothing in this paper depends on it. Restoring the label
requires a round of review that finds nothing, which has not yet happened.
