...
strictly harder (so two--cycles are extremal). Section~\ref{A-sec:anatomy} of the computational companion gives a local congruence
anatomy, a heuristic model, and a proof (via Erd\H{o}s--Wintner) that the relevant abundance density
is a genuine constant. Section~\ref{B-sec:ff} of the obstructions companion adds a function--field lens: over $\mathbb{F}_q[t]$ the
...
circular. Section~\ref{A-sec:breeder} of the computational companion recasts the rule as a bilinear breeder, records a conditional
...
and Proposition~\ref{A-prop:tailkill} together with the search of Remark~\ref{A-rem:notlehmer} closes
...
Corollary~\ref{B-cor:count59}, whose products satisfy $\log_{10}D\le113.397$
...
Remark~\ref{A-rem:notlehmer}, however far they are pushed in $\omega(m)$, bear on level $60$ alone;
...
\end{array}
\]
The sweep of Proposition~\ref{B-prop:nearmiss}, now carried to $10^{8}$, therefore already excludes
...
($\le5$), $\mathbb{F}_5$ ($\le4$), confirming Theorem~\ref{B-thm:ff}.
...
$\delta=0.04202$, agreeing to four places (Proposition~\ref{A-prop:density}).
...
\item Theorem~\ref{B-thm:halflyap} (\texttt{log\_lt\_half\_sub}, \texttt{delta\_decreasing},
\texttt{two\_le\_add\_of\_mul\_eq\_one}) and Theorem~\ref{B-thm:lyapfalse}
...
\item Proposition~\ref{B-prop:masssumthreshold} (\texttt{half\_works}, \texttt{no\_f\_above\_two},
\texttt{mass\_sum\_threshold}) and Proposition~\ref{B-prop:lyapuniversal}
...
branch in Theorem~\ref{B-thm:noinvariant};
\item Proposition~\ref{B-prop:infzero} (\texttt{exists\_block\_sum\_near}, \texttt{infimum\_zero});
\item Theorem~\ref{B-thm:noinvariant}(1) in its mechanism (\texttt{Erdos307.Congruential}) and
...
\emph{Why level $61$ is still not enumerable.} This is the complement of Proposition~\ref{B-prop:window}, which places the mass of a \emph{solution}
...
can be approached, which is the direction Proposition~\ref{B-prop:effapprox} occupies, and says
...
(Proposition~\ref{A-prop:level60closed}), both its sectors closing without factorisation, which sharpens the barrier to
$|P\cup Q|\ge61$ and $\min(\prod P,\prod Q)>5.89\times10^{58}$ (Corollary~\ref{A-cor:level61bound}).
...
and the arithmetic of each result enters only in producing its $c$ (Remark~\ref{B-rem:dictionary}).
Read in the size it gives the double--logarithmic frontier law
(Proposition~\ref{B-prop:frontier}); in the near--miss ratio, the cost family
(Corollary~\ref{B-cor:cost}), whose endpoint at $r=1$ is the barrier and which shows $a''\ge2a$ needs
...
(Proposition~\ref{B-prop:growth}); in the split, that $\min(\omega(a),\omega(b))\ge3$ whenever
$|U|\le68$ (Proposition~\ref{B-prop:split}); and in the level, the forced core and the ceiling that
pin level $59$ from both sides and yield its admissible count $49{,}961$ in closed characterisation
(Propositions~\ref{B-prop:window},~\ref{B-prop:band}, Corollary~\ref{B-cor:count59}). The algebraic cores
...
$P=\log\log N/(\log\log\log N)^{1+\varepsilon}$, where two uniform estimates \emph{are} available, Lemma~\ref{C-lem:charcancelunif} for the character sums and Lemma~\ref{C-lem:deficit} for the local
...
candidate $q$ is always integral (Proposition~\ref{A-prop:bdeint}), a conditional reduction
...
(Proposition~\ref{A-prop:nofamily}), which closes the Th\=abit/Euler programme rather than only its
...
(Proposition~\ref{B-prop:anticorr}); we claim no proof in either direction.

Open problems: (i) close level $61$ as level $60$ was closed (Proposition~\ref{A-prop:level60closed}: the
...
law is explained; it is the reciprocity switch of Remark~\ref{A-rem:fiveeighths}, made exact by
...
$0.04202\le\delta\le0.04223$ of Proposition~\ref{A-prop:density}, whose width is carried almost
...
Section~\ref{A-sec:breeder} of the computational companion, or exhibit a non--circular frame family escaping the $\tau(K)/G$ ceiling;
(v) find a $\mathbb{Z}$--side structure detecting the archimedean obstruction of
Section~\ref{B-sec:ff} of the obstructions companion; one that must be non--local, since by Proposition~\ref{A-prop:nolocal} no single
...
Proposition~\ref{A-prop:subset}, $A(\Sigma-A)=D^2$, satisfies the \emph{Hasse principle as an equation}:
...
$D/p$. Its local conditions are exactly the local anatomy of Lemma~\ref{A-lem:anatomy}: modulo $\ell^2$
...
Proposition~\ref{A-prop:localcomplete} shows the cycle system is soluble modulo every $m$, so it has
solutions in $\widehat{\mathbb{Z}}$ and there is no congruence obstruction to find, at any modulus or
any finite combination of moduli. Theorem~\ref{B-thm:ff} shows the analogue over $\mathbb{F}_q[t]$ is
...
Proposition~\ref{A-prop:barthreshold} shows it is exactly tight: raising the barrier one level requires
...
$(A,B)$ of the twisted two--cycles of $\mathbb{Z}[\sqrt2]$ (Proposition~\ref{B-prop:sunit}), and conditional on
...
the local zero--sum congruences (his Prop.~8.1, our Lemma~\ref{A-lem:anatomy}); the parity restrictions
