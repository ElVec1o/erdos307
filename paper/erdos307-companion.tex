...
\title{Companion to \emph{On the equation $n''=n$ and a problem of Erd\H{o}s and Barbeau}:\\ explorations, closures and the square sieve}
...
The barrier of Section~\ref{K-sec:barrier} bounds a solution; it does not exclude one. This section
...
Section~\ref{K-sec:frame} in its purest form, with the deviation forced to be the perfect square
...
branch. Proposition~\ref{K-prop:tailbound} withdraws the reading. The orbit is genuinely infinite and
...
Superseded by Proposition~\ref{K-prop:tailbound} and Remark~\ref{rem:notlehmer}: the orbit described
...
(Section~\ref{K-sec:computations}).
\end{itemize}
...
The rank--one rigidity (Section~\ref{K-sec:status}) has a sharp consequence for the circle--method
...
Section~\ref{K-sec:computations} and Proposition~\ref{prop:nolocal} (no single modulus obstructs the
...
Section~\ref{K-sec:frame} is not failing for want of a cleverer ansatz.
...
is not merely circular in the sense of Section~\ref{K-sec:frame}: on near--miss frames it is inverted.
...
Section~\ref{K-sec:frame}: the Newton iteration for \#307 exists, is exact, and converges quadratically,
...
Section~\ref{K-sec:frame} in quantitative form. There is also a structural reason the analogy with
...
particular the finite verifications of Proposition~\ref{K-prop:close59} and Remark~\ref{rem:tier60} are
...
Proposition~\ref{K-prop:nopoly} (no polynomial parametrisation) and Proposition~\ref{prop:noplace}
...
Section~\ref{K-sec:status}, would therefore still not close level $60$. The searches of
Propositions~\ref{prop:tailkill} and~\ref{K-prop:close59} are stated over one--new--prime families
...
Section~\ref{K-sec:lean}, \texttt{Erdos307.dprod\_eq\_of\_mass\_eq} and
...
Section~\ref{K-sec:frame} once more, but the relocation is informative, since a finite, degenerating
