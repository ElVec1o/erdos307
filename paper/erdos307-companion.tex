...
families (Remark~\ref{K-rem:lehmer}, where no sieve reaches at all), but a generation beyond present technology; it is
...
particular the finite verifications of Proposition~\ref{K-prop:close59} and Remark~\ref{K-rem:tier60} are
...
weakens the \textsc{yes} lean of Remark~\ref{K-rem:killcount}, and the $2^{-59}$ filter of
...
Remark~\textup{\ref{K-rem:lehmer}}. Before attacking it, two families of approach can be removed:
...
Remark~\textup{\ref{K-rem:lehmer}} is therefore intrinsic, not an artefact of the one-free-prime
...
a sharper statement of the obstruction than Remark~\ref{K-rem:lehmer} gives, and it is the reason the
...
The $34$ immune families of Remark~\textup{\ref{K-rem:campaign}} are precisely the tail families the
...
Pythagorean pair is, by Remark~\textup{\ref{K-rem:lehmer}}, deciding whether
...
Remark~\ref{K-rem:tier60} already gives local solubility everywhere, \emph{rational} solubility
...
semiprime $A_SB_S$: the same cryptographic wall as Remark~\ref{K-rem:campaign}, reached from a new
direction.
...
which Proposition~\ref{K-prop:localcomplete} proves does not exist. A failure would therefore be
...
Propositions~\ref{K-prop:tailkill} and~\ref{K-prop:close59} are stated over one--new--prime families
...
Remark~\ref{K-rem:lehmer}. It is alive but narrower than it was, and the narrowing should not be
mistaken for proximity. Two cautions, both quantitative. The searches of
Remark~\ref{K-rem:notlehmer} came back empty over $3.99\times10^{11}$ candidates, but empty was the
...
than a Lehmer primality question; Remark~\ref{K-rem:notlehmer} then makes that finite search
...
Remark~\ref{K-rem:lehmer} is exactly the analogue here.
...
Pythagorean tail prime, together with the sign selection $k=0$, which Remark~\ref{K-rem:lehmer} shows
...
Remark~\ref{K-rem:notlehmer} makes heuristically empty on all $34$ immune families. The residue is
...
Proposition~\ref{K-prop:localcomplete} proves the cycle system soluble modulo every $m$, so any
...
$2c\not\equiv0\pmod m$ is \emph{not} established. Proposition~\ref{K-prop:localcomplete} settles only
...
\end{theorem}
\begin{proof}
(1) is not proved; see the statement. Proposition~\ref{K-prop:localcomplete} makes the cycle system
...
derivative, which is Proposition~\ref{K-prop:localcomplete} and is arithmetic. Branch (1) is therefore
...
the arguments that survive to $K[t]$, Proposition~\ref{K-prop:localcomplete} removes the congruences,
...
The existence route of Remark~\ref{K-rem:lehmer} and the counting route of
...
Remark~\ref{K-rem:lehmer} the Pell index $n$ determines $q_n=(x_n^2-D_S)/A_S$, a term of a ternary
...
the observation in Remark~\ref{K-rem:lehmer} that a single square condition is of that class. It is also
...
Remark~\ref{K-rem:lehmer} locates the exponential difficulty in the \emph{conjunction} of the two
